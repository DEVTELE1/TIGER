...
TIGERBOT:sadd(TIGER_ID..'moder'..idg,idmsh)  
print('تم رفع '..k..' منشئين')
end
end
if v.MDER then
for k,idmder in pairs(v.MDER) do
TIGERBOT:sadd(TIGER_ID..'modergroup'..idg,idmder)  
...
vardump(idmod)
TIGERBOT:sadd(TIGER_ID..'mods:'..idg,idmod)  
print('تم رفع '..k..' ادمنيه')
end
end
if v.VIP then
for k,idvip in pairs(v.VIP) do
TIGERBOT:sadd(TIGER_ID..'vip:group'..idg,idvip)  
print('تم رفع '..k..' مميزين')
end
end
if v.linkgroup then
if v.linkgroup ~= "" then
TIGERBOT:set(TIGER_ID.."link:group"..idg,v.linkgroup)   
print('تم وضع رابط ')
end
end
end
...
NAME = TIGERBOT:get(TIGER_ID..'group:name'..v) or ''
NAME = NAME:gsub('"','')
NAME = NAME:gsub('#','')
NAME = NAME:gsub([[\]],'')
link = TIGERBOT:get(TIGER_ID.."link:group"..v) or ''
welcome = TIGERBOT:get(TIGER_ID..'welcome:'..v) or ''
MNSH = TIGERBOT:smembers(TIGER_ID..'moder'..v)
MDER = TIGERBOT:smembers(TIGER_ID..'modergroup'..v)
MOD = TIGERBOT:smembers(TIGER_ID..'mods:'..v)
VIP = TIGERBOT:smembers(TIGER_ID..'vip:group'..v)
if k == 1 then
t = t..'"'..v..'":{"GP_NAME":"'..NAME..'",'
else
t = t..',"'..v..'":{"GP_NAME":"'..NAME..'",'
...
local u = TIGERBOT:get(TIGER_ID.."user:Name" .. v) or '@TIGER_SOURCE'
if k == 1 then
t =  t..'"'..v..'"'
else
t =  t..',"'..v..'"'
