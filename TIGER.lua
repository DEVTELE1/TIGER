...
TIGERBOT = redis.connect('127.0.0.1', 6379) 
...
DEVTIGER = io.popen("echo $SSH_CLIENT | awk '{ print $1}'"):read('*a')
...
local serialized   
if not uglify then  
serialized = serpent.block(data, {comment = false, name = "TIGER_INFO"})  
else  
...
if not TIGERBOT:get(DEVTIGER..":token") then
...
TIGERBOT:set(DEVTIGER..":token",token)
end 
else
...
if not TIGERBOT:get(DEVTIGER..":SUDO:ID") then
io.write('\27[1;31m ↡ ارسل ايدي مطور الاساسي |\n SEND ID FOR SIDO : \27[0;39;49m')
local SUDOID = io.read()
if SUDOID ~= '' then
io.write('\n\27[1;34m تم حفظ ايدي المطور |\n\27[0;39;49m')
TIGERBOT:set(DEVTIGER..":SUDO:ID",SUDOID)
else
print('\n\27[1;34m لم يتم حفظ ايدي المطور |')
end 
os.execute('lua TIGER.lua')
end
if not TIGERBOT:get(DEVTIGER..":SUDO:USERNAME") then
io.write('\27[1;31m ↡ ارسل معرف المطور الاساسي |\n SEND ID FOR SIDO : \27[0;39;49m')
local SUDOUSERNAME = io.read():gsub('@','')
if SUDOUSERNAME ~= '' then
io.write('\n\27[1;34m تم حفظ معرف المطور |\n\27[0;39;49m')
TIGERBOT:set(DEVTIGER..":SUDO:USERNAME",'@'..SUDOUSERNAME)
else
...
config = {
token = TIGERBOT:get(DEVTIGER..":token"),
SUDO = TIGERBOT:get(DEVTIGER..":SUDO:ID"),
USERNAME = TIGERBOT:get(DEVTIGER..":SUDO:USERNAME"), }
...
local load_TIGERBOT = function()  
local f = io.open("./requfiles/INFO.lua", "r")  
if not f then   
AutoSet()  
else   
f:close()  
TIGERBOT:del(DEVTIGER..":token")
TIGERBOT:del(DEVTIGER..":SUDO:ID")
TIGERBOT:del(DEVTIGER..":SUDO:USERNAME")
TIGERBOT:del(DEVTIGER..":NAMEBOT")
end  
local config = loadfile("./requfiles/INFO.lua")() 
return config 
end 
_TIGERBOT = load_TIGERBOT()  
...
local hash = TIGERBOT:sismember(TIGER_ID..'sudo:bot',msg.sender_user_id_)  
...
local hash = TIGERBOT:sismember(TIGER_ID..'moder'..msg.chat_id_,msg.sender_user_id_)    
...
local hash = TIGERBOT:sismember(TIGER_ID..'modergroup'..msg.chat_id_,msg.sender_user_id_)    
...
local hash = TIGERBOT:sismember(TIGER_ID..'SET:BAN'..msg.chat_id_,msg.sender_user_id_)    
if hash or  is_devtaha(msg) or is_sudo(msg) or is_owner(msg) then    
return true    
else    
return false    
...
function is_mod(msg) 
local hash = TIGERBOT:sismember(TIGER_ID..'mods:'..msg.chat_id_,msg.sender_user_id_)    
...
local hash = TIGERBOT:sismember(TIGER_ID..'vip:groups',msg.sender_user_id_) 
if hash or  is_devtaha(msg) then 
return true 
else 
return false 
...
local hash = TIGERBOT:sismember(TIGER_ID..'vip:group'..msg.chat_id_,msg.sender_user_id_) 
...
local hash = TIGERBOT:sismember(TIGER_ID..'mepar',msg.sender_user_id_) 
...
local hash =  TIGERBOT:sismember(TIGER_ID..'TIGER:baned'..chat,user) 
if hash then return true 
else 
return false 
end end
function is_gban(chat,user) 
local hash =  TIGERBOT:sismember(TIGER_ID..'TIGER:gbaned',user) 
if hash then 
return true 
else 
return false 
...
function TIGER_sendMsg(chat_id, replytomessageid, from_background, text, DisableWebPagePreview, parsemode, user, cd, TIGER)
...
Entities[0] = {ID='MessageEntityBold', offset_=0, length_=0}
tdcli_function ({ID = "SendMessage",chat_id_ = chat_id,reply_to_message_id_ = replytomessageid or 0,disable_notification_ = 0,from_background_ = from_background,reply_markup_ = nil,input_message_content_ = {ID = "InputMessageText",text_ = text,disable_web_page_preview_ = DisableWebPagePreview,clear_draft_ = 0,entities_ = Entities,parse_mode_ = parsemode,},}, cd or dl_cb,TIGER or nil)
end
function sleep(n) os.execute("sleep " .. tonumber(n)) end  
function add_in_ch(msg)
local var = true
if TIGERBOT:get(TIGER_ID..'add:ch:id') then
tahaid = TIGERBOT:get(TIGER_ID..'add:ch:id')
tahauser = TIGERBOT:get(TIGER_ID..'add:ch:username')
...
var = false
if TIGERBOT:get(TIGER_ID..'text:ch:user') then
...
local sudoe = TIGERBOT:sismember(TIGER_ID..'sudo:bot',user) 
local noow = TIGERBOT:sismember(TIGER_ID..'moder'..chat,user) 
...
local moder =  TIGERBOT:sismember(TIGER_ID..'modergroup'..chat,user) 
local mod TIGERBOT:sismember(TIGER_ID..'mods:'..chat,user) 
...
local function callback_admins(extra,result,success)    limit = (result.member_count_ ) getChannelMembers(channel, 0, 'Bots', limit,cb)    TIGER_sendMsg(channel, 0, 1,'💢*┇* تم طرد البوتات \n', 1, 'md') end  getChannelFull(channel,callback_admins) 
end
...
TIGERBOT:set(TIGER_ID.."user:Name"..id, "@"..username)
print('\27[30;35m»» USERNAME IS SAEVE ↓\n»» @'..data.username_..'\n\27[1;37m') else
TIGERBOT:del(TIGER_ID.."user:Name"..id)
print('»» USERNAME IS NOT SAEVE') end
if data.type_.ID == "UserTypeBot" then
TIGERBOT:del(TIGER_ID.."user:Name"..id) end
...
local function callback_admins(extra,result,success)   limit = result.administrator_count_  if tonumber(limit) > 0 then    getChannelMembers(channel, 0, 'Administrators', limit,cb)     else return TIGER_sendMsg(channel, 0, 1,'', 1, 'md') end    end  getChannelFull(channel,callback_admins) 
end
function TIGERset(chat_id, TIGERphoto) 
tdcli_function ({ ID = "ChangeChatPhoto",chat_id_ = chat_id,photo_ = getInputFile(TIGERphoto) }, dl_cb, nil) 
end
...
t = 'المطور'
elseif TIGERBOT:sismember(TIGER_ID..'sudo:bot',user_id) then
t = 'المطور'
elseif TIGERBOT:sismember(TIGER_ID..'moder'..chat_id,user_id) then
t = 'المنشئ'
elseif TIGERBOT:sismember(TIGER_ID..'modergroup'..chat_id,user_id) then
t = 'المدير'
elseif TIGERBOT:sismember(TIGER_ID..'mods:'..chat_id,user_id) then
t = 'الادمن'
elseif TIGERBOT:sismember(TIGER_ID..'vip:groups',user_id) then
t = 'المميز عام'
elseif TIGERBOT:sismember(TIGER_ID..'vip:group'..chat_id,user_id) then
t = 'المميز'
else
t = ' '
end
...
if TIGERBOT:sismember(TIGER_ID..'modergroup'..chat_id,user_id) then
t = 'المدير'
elseif TIGERBOT:sismember(TIGER_ID..'mods:'..chat_id,user_id) then
t = 'الادمن'
elseif TIGERBOT:sismember(TIGER_ID..'vip:group'..chat_id,user_id) then
...
TIGERBOT:del(TIGER_ID.."lock:user:name"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:hashtak"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:Cmd"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:Link"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:forward"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:Keyboard"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:geam"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:Photo"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:Animation"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:Video"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:Audio"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:vico"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:Sticker"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:Document"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:Unsupported"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:Markdaun"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:Contact"..chat_id)  TIGERBOT:del(TIGER_ID.."lock:Spam"..chat_id)
end
local function add_lockal(chat_id)
TIGERBOT:set(TIGER_ID.."lock:Bot:kick"..chat_id,'del')  TIGERBOT:set(TIGER_ID..'lock:tagservrbot'..chat_id,true)   TIGERBOT:set(TIGER_ID.."lock:user:name"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:hashtak"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:Cmd"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:Link"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:forward"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:Keyboard"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:geam"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:Photo"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:Animation"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:Video"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:Audio"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:vico"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:Sticker"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:Document"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:Unsupported"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:Markdaun"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:Contact"..chat_id,'del')    TIGERBOT:set(TIGER_ID.."lock:Spam"..chat_id,'del')  
end
function rem_group(chat_id) 
TIGERBOT:srem(TIGER_ID..'bot:gpsby:id',chat_id) TIGERBOT:del(TIGER_ID.."test:group"..chat_id)   TIGERBOT:srem(TIGER_ID.."bot:gps:id", chat_id) TIGERBOT:del(TIGER_ID.."add:bot:group"..chat_id,true) 
rem_lockal(chat_id) 
end
function add_group(chat_id) 
TIGERBOT:sadd(TIGER_ID..'bot:gpsby:id',chat_id) TIGERBOT:sadd(TIGER_ID.."botgps",chat_id)  TIGERBOT:set(TIGER_ID.."test:group"..chat_id,'TIGER')    TIGERBOT:set(TIGER_ID.."add:bot:group"..chat_id, true)   
add_lockal(chat_id)
end
...
local names = TIGERBOT:smembers(TIGER_ID..'filters:'..msg.chat_id_)  local var = false for i=1, #names do  if names[i] == value and not is_vipgroup(msg) then  var = true      end end return var  
end 
function is_muted(chat,user)
local hash =  TIGERBOT:sismember(TIGER_ID..'mutes'..chat,user) if hash then return true else return false end 
end
function TIGERmonshn(chat_id, user_id, msg_id, text, offset, length) 
...
function TIGER1(chat,user) 
local sudoe = TIGERBOT:sismember(TIGER_ID..'sudo:bot',user) local vipss = TIGERBOT:sismember(TIGER_ID..'vip:groups',user)  local monh = TIGERBOT:sismember(TIGER_ID..'modergroup'..chat,user)  local noow = TIGERBOT:sismember(TIGER_ID..'moder'..chat,user)  local nomo = TIGERBOT:sismember(TIGER_ID..'mods:'..chat,user)  local novip2 = TIGERBOT:sismember(TIGER_ID..'vip:group'..chat,user)  if tonumber(SUDO) == tonumber(user) or sudoe or vipss or monh  or noow or nomo  or novip2 then  return true  else  return false  end  
end 
function TIGER2(chat,user) 
local sudoe = TIGERBOT:sismember(TIGER_ID..'sudo:bot',user) local vipss = TIGERBOT:sismember(TIGER_ID..'vip:groups',user)  local noow = TIGERBOT:sismember(TIGER_ID..'moder'..chat,user)  if tonumber(SUDO) == tonumber(user) or sudoe or vipss or noow  then  return true else  return false end 
end 
function TIGER3(chat,user) 
local sudoe = TIGERBOT:sismember(TIGER_ID..'sudo:bot',user) local noow = TIGERBOT:sismember(TIGER_ID..'moder'..chat,user)  if tonumber(SUDO) == tonumber(user) or sudoe or monh   then return true else return false end 
end 
function TIGER4(chat,user) 
local sudoe = TIGERBOT:sismember(TIGER_ID..'sudo:bot',user) local noow = TIGERBOT:sismember(TIGER_ID..'moder'..chat,user)  local monh = TIGERBOT:sismember(TIGER_ID..'modergroup'..chat,user)     local memr = TIGERBOT:sismember(TIGER_ID..'mepar',user)    if tonumber(SUDO) == tonumber(user) or sudoe or monh or memr or noow then     return true     else     return false     end     
end 
function getInputFile(file) 
...
function send_inlineTIGER(chat_id,text,keyboard,inline,reply_id) 
local url = 'https://api.telegram.org/bot'..chaneel if keyboard then TIGERtoken = url .. '/sendMessage?chat_id=' ..chat_id.. '&text='..URL.escape(text)..'&parse_mode=html&reply_markup='..URL.escape(json:encode(keyboard)) else TIGERtoken = url .. '/sendMessage?chat_id=' ..chat_id.. '&text=' ..URL.escape(text)..'&parse_mode=html' end https.request(TIGERtoken) 
end
...
local function TIGER_sendMssg(chat_id, text, reply_to_message_id, markdown) 
...
tdcli_function ({ ID = "GetChannelFull",channel_id_ = getChatId(GroupID).ID },function(arg,tah)   TIGERBOT:set(TIGER_ID.."numgrop"..GroupID,tah.member_count_) end,nil)  return TIGERBOT:get(TIGER_ID.."numgrop"..GroupID)   
end
local function title_name(GroupID) 
pcall(tdcli_function({ID ="GetChat",chat_id_=GroupID },function(arg,data)  TIGERBOT:set(TIGER_ID..'group:name'..GroupID,data.title_)  end,nil)) return TIGERBOT:get(TIGER_ID..'group:name'..GroupID)  
end
...
if tonumber(user) == tonumber(bot_id) then  return false  end  if TIGER1(chat,user) then  else  changeChatMemberStatus(chat, user, "Kicked")  tdcli_function({ID="ChangeChatMemberStatus",chat_id_=chat,user_id_=user,status_={ID="ChatMemberStatusLeft"}},function(arg,ta) end,nil) end  
end
...
local get_id = text local get_id = get_id:gsub('IDGET',msg.sender_user_id_) local get_id = get_id:gsub('USERGET',USERNAME_GET) local get_id = get_id:gsub('RTBGET',t) local get_id = get_id:gsub('RTGGET',rtpa) local get_id = get_id:gsub('MSGGET',tonumber(TIGERBOT:get(TIGER_ID..'user:messages:'..msg.chat_id_..':'..msg.sender_user_id_) or 1)) local get_id = get_id:gsub('TFGET',formsgg(tonumber(TIGERBOT:get(TIGER_ID..'user:messages:'..msg.chat_id_..':'..msg.sender_user_id_) or 1))) local get_id = get_id:gsub('PHOTOGET',taha.total_count_) local get_id = get_id:gsub('NKOGET',nko)
return get_id 
end
local function monsendwel(msg,chat,text,user)   
entities = {}   entities[0] = {ID='MessageEntityBold', offset_=0, length_=0}   if text and text:match('<TIGER>') and text:match('</TIGER>')  then   local x = utf8.len(text:match('(.*)<TIGER>'))   local offset = x + 1  local y = utf8.len(text:match('<TIGER>(.*)</TIGER>'))   local length = y + 1  text = text:gsub('<TIGER>','')   text = text:gsub('</TIGER>','')   table.insert(entities,{ID="MessageEntityMentionName", offset_=offset, length_=length, user_id_=user})   end   if text and text:match('❛') and text:match('❜') then   local x = utf8.len(text:match('(.*)❛'))   local offset = x   local y = utf8.len(text:match('❛(.*)❜'))   local length = y   text = text:gsub('❛','')   text = text:gsub('❜','')   table.insert(entities,{ID="MessageEntityBold", offset_=offset, length_=length})   end   return tdcli_function ({ID="SendMessage", chat_id_=chat, reply_to_message_id_=msg.id_, disable_notification_=0, from_background_=1, reply_markup_=nil, input_message_content_={ID="InputMessageText", text_=text, disable_web_page_preview_=1, clear_draft_=0, entities_=entities}}, dl_cb, nil)   
end
local function addmod(chat_id)   
tdcli_function ({ID = "GetChannelMembers", channel_id_ = getChatId(chat_id).ID,filter_ = {ID = "ChannelMembersAdministrators"},offset_ = 0,limit_ = 100 },function(arg,data)  local admins = data.members_ for i=0 , #admins do  if data.members_[i].status_.ID == "ChatMemberStatusCreator" then owner_id = admins[i].user_id_  TIGERBOT:sadd(TIGER_ID..'moder'..chat_id,owner_id)   tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_ },function(arg,b)   if b.username_ == true then  TIGERBOT:set(TIGER_ID.."user:Name"..b.id_,"@"..b.username_) end end,nil)    end  if data.members_[i].bot_info_ == false and data.members_[i].status_.ID == "ChatMemberStatusEditor" then  TIGERBOT:sadd(TIGER_ID..'mods:'..chat_id,admins[i].user_id_)   tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_ },function(arg,b)   if b.username_ == true then  TIGERBOT:set(TIGER_ID.."user:Name"..b.id_,"@"..b.username_) end end,nil)    else  TIGERBOT:srem(TIGER_ID..'mods:'..chat_id,admins[i].user_id_)   end end end,nil)     
end
...
kick(msg,msg.chat_id_,msg.sender_user_id_) 
TIGERmonshn(msg.chat_id_, msg.sender_user_id_, msg.id_, '👤┇ العضــو » ( '..CatchName(data.first_name_,20)..' )\n📬┇ قام بالتكرار هنا وتم طرده \n' , 14, utf8.len(data.first_name_)) 
end
return false  
end 
if type == 'del' then 
...
TIGERBOT:sadd(TIGER_ID..'tedmembars'..msg.chat_id_,msg.sender_user_id_) 
else
TIGERmonshn(msg.chat_id_, msg.sender_user_id_, msg.id_, '👤┇ العضــو » ( '..CatchName(data.first_name_,20)..' )\n📬┇ قام بالتكرار هنا وتم تقييده \n' , 14, utf8.len(data.first_name_)) 
HTTPS.request("https://api.telegram.org/bot" .. chaneel .. "/restrictChatMember?chat_id=" ..msg.chat_id_.. "&user_id=" ..msg.sender_user_id_.."") 
TIGERBOT:sadd(TIGER_ID..'tedmembars'..msg.chat_id_,msg.sender_user_id_) 
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, taha, 1, "md")  
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_) 
else
TIGERmonshn(msg.chat_id_, msg.sender_user_id_, msg.id_, '👤┇ العضــو » ( '..CatchName(data.first_name_,20)..' )\n📬┇ قام بالتكرار هنا وتم كتمه \n' , 14, utf8.len(data.first_name_))  
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_) 
end
...
if plugin.TIGER_TEXT and msg then
print('\27[30;35m The Msg Process :'..name..'\n\27[1;37m')
pre_msg = plugin.TIGER_TEXT(msg)
end
end
return TIGER_sendMsg(msg.chat_id_, msg.id_, 1,pre_msg, 1, 'md')  
end
...
if plugin.TIGER then
local TEXT = plugin.TIGER(msg,MSG_TEXT)
if TEXT then
...
if msg.content_.ID == "MessageChatAddMembers" and not TIGERBOT:get(TIGER_ID..'lock:klsh:add'..bot_id) then   
for i=0,#msg.content_.members_ do    
addbotgrop = msg.content_.members_[i].id_    
if addbotgrop and addbotgrop == tonumber(bot_id) then  
local photo = TIGERBOT:get("addreply1photo1"..bot_id)   
if photo then  
local klishwelc = TIGERBOT:get("klish:welc"..bot_id)  
...
TIGERBOT:del("klish:welc"..bot_id)  
TIGERBOT:del("addreply1photo1"..bot_id)   
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*📮┇ تم مسح كليشه ترحيب عند اضافه البوت *\n', 1, 'md') 
end

if text == 'تحديث' and is_devtaha(msg) then  
dofile('TIGER.lua')  
load_plugins()
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*📮┇ تم تحديث الملفات ♻* \n', 1, 'md') 
end 
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*📮┇ تنبيــــه لا استطيع تفعيل المجموعه انا لست ادمن يرجى ترقيتي ادمن *\n✔🙋🏻‍♂️※\n", 1, "md") 
return false  end 
...
TIGERBOT:set(TIGER_ID.."add:bot:group"..msg.chat_id_, true)  
TIGERBOT:sadd(TIGER_ID..'bot:gpsby:id', msg.chat_id_)   
TIGERBOT:sadd(TIGER_ID..'bot:gpsby:id:add', msg.chat_id_)   
TIGERBOT:sadd(TIGER_ID.."botgps", msg.chat_id_)  
...
local numadd = TIGERBOT:scard(TIGER_ID..'add:num'..msg.sender_user_id_)
...
'\n  ٴ━━━━━━━━━━'..
'\n✔🙋🏻‍♂️※┇ معلومات '..rtpa_add..' الي فعل المجموعه '..
'\n🔸┇ الايدي » ❪`'..msg.sender_user_id_..'`❫'..usersdd..
'\n🔸┇ الاسم » ❪['..CatchName(result.first_name_,45)..'](tg://user?id='..msg.sender_user_id_..')❫'
TIGER_sendMssg(SUDO,text,0,'md')
end end end,nil) end,nil)   
end
else
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*❕┇ انت لست ادمن في المجموعه *\n✔🙋🏻‍♂️※\n", 1, "md") 
end
end,nil)   
end,nil) 
end
...
TIGERBOT:set(TIGER_ID.."test:group"..msg.chat_id_,'TIGER')   
end  
...
TIGERBOT:srem(TIGER_ID.."bot:gps:id", msg.chat_id_)  
TIGERBOT:srem(TIGER_ID.."botgps", msg.chat_id_)  
TIGERBOT:srem(TIGER_ID..'add:num'..msg.sender_user_id_,msg.chat_id_) 
TIGERBOT:del(TIGER_ID.."test:group" .. msg.chat_id_)
...
function TIGER(msg,data) 
if msg then 
if msg.date_ and msg.date_ < tonumber(os.time() - 30) then
print('OLD MESSAGE')
return false
end
TIGERBOT:incr(TIGER_ID..'groupmsg:'..msg.chat_id_..':') 
TIGERBOT:incr(TIGER_ID..'user:messages:'..msg.chat_id_..':'..msg.sender_user_id_) 
TIGERBOT:incr(TIGER_ID..'msg:chat:'..msg.chat_id_..':') 
TIGERBOT:incr(TIGER_ID..'msg:user:'..msg.chat_id_..':'..msg.sender_user_id_)
if msg.send_state_.ID == "MessageIsSuccessfullySent" then return false  end end
if msg.can_be_deleted_ == true and not is_mod(msg) then   tdcli_function ({ ID = "GetUser", user_id_ = msg.sender_user_id_ },function(arg,data)  if data.username_ == true then TIGERBOT:set(TIGER_ID.."user:Name"..msg.sender_user_id_,"@"..data.username_) end end,nil)
if TIGERBOT:get(TIGER_ID..'add:mepr:'..msg.chat_id_) and tonumber(TIGERBOT:scard(TIGER_ID..'addedtaha:'..msg.sender_user_id_)) == tonumber(TIGERBOT:get(TIGER_ID..'setadd:'..msg.chat_id_)) then  
TIGERBOT:sadd(TIGER_ID..'meaddwy:'..msg.chat_id_,msg.sender_user_id_)  
TIGERBOT:del(TIGER_ID.."addedtaha:"..msg.sender_user_id_)  
TIGERBOT:srem(TIGER_ID..'meaddtaha:'..msg.chat_id_,msg.sender_user_id_)  
tdcli_function ({
ID = "GetUser",
...
elseif msg.content_.text_ and TIGERBOT:get(TIGER_ID..'add:mepr:'..msg.chat_id_) and not TIGERBOT:sismember(TIGER_ID..'meaddwy:'..msg.chat_id_,msg.sender_user_id_) and not TIGERBOT:sismember(TIGER_ID..'meaddtaha:'..msg.chat_id_,msg.sender_user_id_) then  
if tonumber(TIGERBOT:scard(TIGER_ID..'addedtaha:'..msg.sender_user_id_)) ~= tonumber(TIGERBOT:get(TIGER_ID..'setadd:'..msg.chat_id_)) then  
TIGERBOT:sadd(TIGER_ID..'meaddtaha:'..msg.chat_id_,msg.sender_user_id_)  
local mustadd = (TIGERBOT:get(TIGER_ID..'setadd:'..msg.chat_id_) or '0')  
tdcli_function ({
ID = "GetUser",
user_id_ = msg.sender_user_id_
},function(arg,data) 
...
for i=0,#mem_id do  
if TIGERBOT:get(TIGER_ID..'add:mepr:'..msg.chat_id_) then  
if not TIGERBOT:sismember(TIGER_ID..'meaddwy:'..msg.chat_id_,msg.sender_user_id_) then  
if mem_id[i].username_ and mem_id[i].username_:match("[Bb][Oo][Tt]$") then  
return false  
else  
TIGERBOT:sadd(TIGER_ID..'addedtaha:'..msg.sender_user_id_,mem_id[i].id_)  
end  
end  
end  
end  
end  
if TIGERBOT:get(TIGER_ID..'add:mepr:'..msg.chat_id_) and not TIGERBOT:sismember(TIGER_ID..'meaddwy:'..msg.chat_id_,msg.sender_user_id_) then  
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*✔🙋🏻‍♂️※┇* عذرا انا لست ادمن هنا \n', 1, 'md') 
TIGERBOT:del(TIGER_ID..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_) 
return false  end
if data.message_ == "CHAT_ADMIN_REQUIRED" then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*📮┇* عذرا ليست لدي صلاحيه تغير معلومات المجموعه \n', 1, 'md') 
TIGERBOT:del(TIGER_ID..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_) 
else
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🎆┇* تم تغيير صورة المجموعه \n✓※', 1, 'md') 
end
end, nil) 
TIGERBOT:del(TIGER_ID..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_) 
end   
...
chat_type = 'super'
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,data)   TIGERBOT:set(TIGER_ID..'group:name'..msg.chat_id_,data.title_) end,nil) 
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data)  if data.username_ == true then TIGERBOT:set(TIGER_ID.."user:Name"..msg.sender_user_id_,"@"..data.username) end end,nil) 
if msg.can_be_deleted_ == true then
end
elseif id:match('^(%d+)') then 
chat_type = 'user' 
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data)  if data.username_ == true then TIGERBOT:set(TIGER_ID.."user:Name"..msg.sender_user_id_,"@"..data.username) else TIGERBOT:del(TIGER_ID.."user:Name"..msg.sender_user_id_)  end end,nil) 
TIGERBOT:sadd(TIGER_ID.."usersbot",msg.chat_id_)   
else 
chat_type = 'group' 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '📮*┇* البوت لا يدعم المجموعات العاديه تم المغادره 🚨\n', 1, 'md')
changeChatMemberStatus(msg.chat_id_, bot_id, "Left")
end 
end
local text = msg.content_.text_
if msg.content_.ID == "MessageChatAddMembers" then 
TIGERBOT:incr(TIGER_ID..'add:mempar'..msg.chat_id_..':'..msg.sender_user_id_) 
end
...
local nummsgg = TIGERBOT:get(TIGER_ID..'groupmsg:'..ch..':') 
...
'❫*'..(linkgp)..usergp
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,whogp, 1, 'md') 
end,nil) 
end
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*📮┇ البوت ليس ادمن في القناة يرجى ترقيته ادمن ثم اعادة المحاوله *\n✔🙋🏻‍♂️※', 1, 'md') 
end
return false  end
end,nil)
end
...
TIGERBOT:set(TIGER_ID..'text:ch:user',texxt)
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*📮┇ تم تغيير رسالة الاشتراك بنجاح *\n✓※', 1, 'md')
end
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*📬┇ ليس لدي صلاحية دعوة مستخدمين من الرابط *\n✓※", 1, "md") 
return false  
end    
if data and data.channel_ and data.channel_.username_ ~= false then
...
TIGERBOT:set(TIGER_ID.."numgrop"..ch,data.member_count_) 
tdcli_function({ID ="GetChat",chat_id_=ch},function(arg,ata) TIGERBOT:set(TIGER_ID..'group:name'..ch,ata.title_) end,nil)
end
end    
...
return false  end 
TIGERBOT:del(TIGER_ID..'namebot:witting'..msg.sender_user_id_) 
TIGERBOT:set(TIGER_ID..'TIGER:name',text) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*♻┇* تم تغير اسم البوت \n",1, 'md')  
end 
...
TIGERBOT:set(TIGER_ID..'setadd:bot'..bot_id,numadded)  TIGER_sendMsg(msg.chat_id_, msg.id_,  1, "📬*┇* تم وضع عدد الاضافه *{ "..numadded..' }*', 1, 'md')  
end
...
local gps = TIGERBOT:scard(TIGER_ID..'bot:gpsby:id')   
local list = TIGERBOT:smembers(TIGER_ID..'botgps')   
...
local list = TIGERBOT:smembers(TIGER_ID..'usersbot')   
for k,v in pairs(list) do  
forwardMessages(v, msg.chat_id_, {[0] = msg.id_}, 1)  
end   
end 
local gpspv = TIGERBOT:scard(TIGER_ID..'usersbot')   
local gps = TIGERBOT:scard(TIGER_ID..'bot:gpsby:id')   
local gmfwd = '📮*┇ تمت اذاعة الى *'..
'\n*⚀┇ » ❪'..gpspv..'❫* مشترك في الخاص'..
'\n*⚁ ※ » ❪'..gps..'❫* مجموعه في البوت\n✔🙋🏻‍♂️※' 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, gmfwd, 1, 'md')
TIGERBOT:del(TIGER_ID.."bc:in:allfwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
end
...
return false  end 
if msg.forward_info_ then 
local gps = TIGERBOT:scard(TIGER_ID..'usersbot')   
local list = TIGERBOT:smembers(TIGER_ID..'usersbot')   
for k,v in pairs(list) do  
forwardMessages(v, msg.chat_id_, {[0] = msg.id_}, 1)  
end   
...
whatbc = 'الرساله'
local list = TIGERBOT:smembers(TIGER_ID..'bot:gpsby:id') 
for k,v in pairs(list) do 
TIGER_sendMsg(v, 0, 1, '[ '..msg.content_.text_..' ]', 1, 'md')  
end
...
local gpsv = TIGERBOT:scard(TIGER_ID.."usersbot") or 0 
if msg.content_.text_ then
whatbc = 'الرساله'
local list = TIGERBOT:smembers(TIGER_ID..'usersbot') 
for k,v in pairs(list) do 
...
if text == '/start' then  
if is_devtaha(msg) then
local taha = '*📮┇ اهلا بك عزيزي المطور ✔🙋🏻‍♂️※'..
...
if not TIGERBOT:get(TIGER_ID..'START:STOP'..msg.sender_user_id_) then
...
TIGERBOT:sadd(TIGER_ID.."usersbot",msg.chat_id_)   
local start = TIGERBOT:get(TIGER_ID.."start:msgofstart1")  
if start then 
...
TIGERBOT:setex(TIGER_ID..'START:STOP'..msg.sender_user_id_,300,true)
end 
...
if tonumber(TIGERBOT:get('numlocktextpv'..bot_id..msg.sender_user_id_) or 1) >= 100 then      
return false  end   
end  
if not is_devtaha(msg) then
if msg.forward_info_ and  TIGERBOT:get(TIGER_ID..'lock:fwd'..bot_id) then     
return false  end    
if  msg.content_.ID == 'MessagePhoto' and TIGERBOT:get(TIGER_ID..'lock:photo'..bot_id) then      
return false  end     
if msg.content_.ID == 'MessageDocument' and  TIGERBOT:get(TIGER_ID..'lock:file'..bot_id) then     
return false  end    
if msg.content_.ID == 'MessageSticker' and  TIGERBOT:get(TIGER_ID..'lock:ste'..bot_id) then     
return false  end    
if msg.content_.ID == 'MessageVoice' and  TIGERBOT:get(TIGER_ID..'lock:musec'..bot_id) then     
return false  end    
if msg.content_.ID == 'MessageContact' and  TIGERBOT:get(TIGER_ID..'lock:phon'..bot_id) then     
return false  end    
if msg.content_.ID == 'MessageVideo' and  TIGERBOT:get(TIGER_ID..'lock:ved'..bot_id) then     
return false  end    
if msg.content_.ID == 'MessageAnimation' and  TIGERBOT:get(TIGER_ID..'lock:gif'..bot_id) then     
return false  end    
if msg.content_.ID == 'MessageAudio' and  TIGERBOT:get(TIGER_ID..'lock:vico'..bot_id) then     
return false  end    
...
if TIGERBOT:get(TIGER_ID..'lock:links'..bot_id) then      
return false  end  
end
end  
if not TIGERBOT:get(TIGER_ID..'lock:botl'..bot_id) then    
if not is_devtaha(msg) then     
local pvstart = TIGERBOT:get(TIGER_ID.."pvstart")    
if pvstart then    
...
TIGERmonshn(SUDO, msg.sender_user_id_, 0, text, 32, utf8.len(ta.first_name_)) 
else
zo = '📬┇ تم ارسال الملصق \n*📮┇ من ↫ ❪ *[@'..ta.username_..'] ❫\n✓'
TIGER_sendMsg(SUDO, 0, 1, zo, 1, "md") 
end end end,nil) end,nil)
end end end    
...
if not TIGERBOT:sismember(TIGER_ID..'pv:ban'..msg.chat_id_,id_user) then
if data.username_ == false then
local text = '👤┇ العضــو ↫ ❪ '..CatchName(data.first_name_,15)..' ❫\n📬┇ تم حظره من التواصل\n✓'
TIGERBOT:incrby('numlocktextpv'..bot_id..id_user,10000000)    TIGERBOT:sadd(TIGER_ID..'pv:ban'..msg.chat_id_,id_user) 
...
return false  end 
if text then    
TIGER_sendMsg(id_user,msg.id_,  1,  text, 1, 'html')    
if data.username_ == false then
local text = '📬┇ تم ارسال رسالتك \n📮┇ الى ↫ ❪ '..CatchName(data.first_name_,15)..' ❫\n✓'
TIGERmonshn(msg.chat_id_, id_user, msg.id_, text, 33, utf8.len(CatchName(data.first_name_,15))) 
else
zo = '📬┇ تم ارسال رسالتك \n*📮┇ الى ↫ ❪ *[@'..data.username_..'] ❫\n✓'
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, zo, 1, "md") 
end
...
TIGERmonshn(msg.chat_id_, id_user, msg.id_, text, 35, utf8.len(CatchName(data.first_name_,15))) 
else
zo = '📬┇ تم ارسال رسالتك \n*📮┇ الى ↫ ❪ *[@'..data.username_..'] ❫\n✓'
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, zo, 1, "md") 
end
...
TIGERmonshn(msg.chat_id_, id_user, msg.id_, text, 32, utf8.len(CatchName(data.first_name_,15))) 
else
zo = '📬┇ تم ارسال رسالتك \n*📮┇ الى ↫ ❪ *[@'..data.username_..'] ❫\n✓'
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, zo, 1, "md") 
end
...
TIGERmonshn(msg.chat_id_, id_user, msg.id_, text, 38, utf8.len(CatchName(data.first_name_,15))) 
else
zo = '📬┇ تم ارسال رسالتك \n*📮┇ الى ↫ ❪ *[@'..data.username_..'] ❫\n✓'
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, zo, 1, "md") 
end
...
if text == 'مسح المحظورين 📢' and is_devtaha(msg) then       local list = TIGERBOT:smembers(TIGER_ID..'pv:ban'..msg.chat_id_)   for k,v in pairs(list) do     TIGERBOT:del(TIGER_ID..'pv:ban'..msg.chat_id_)   TIGERBOT:del('numlocktextpv'..bot_id..v)  end   TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'📛*┇* تم مسح المحظورين', 1, 'md')     end
if text == 'تفعيل البوت الخدمي 🎮' and is_devtaha(msg) then local  taha = '*📛┇*تم تفعيل البوت خدمي  ✔' TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") TIGERBOT:del(TIGER_ID..'lock:bot:free'..bot_id) end 
if text == 'تعطيل البوت الخدمي 🚸' and is_devtaha(msg) then taha = '*📛┇*تم تعطيل البوت الخدمي  ❌' TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") TIGERBOT:set(TIGER_ID..'lock:bot:free'..bot_id,true) end
if text == 'تفعيل تواصل 📨' and is_devtaha(msg) then local  taha = '*📛┇*تم تفعيل بوت التواصل  ✔' TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") TIGERBOT:del(TIGER_ID..'lock:botl'..bot_id) end 
if text == 'تعطيل تواصل 📩' and is_devtaha(msg) then taha = '*📛┇*تم تعطيل التواصل  ❌' TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") TIGERBOT:set(TIGER_ID..'lock:botl'..bot_id,true) end
if text == 'تحديث ♻' and is_devtaha(msg) then  local filed = io.popen('ls plugins_'):lines() for files in filed do if files:match(".lua$") then end end dofile('TIGER.lua') load_plugins() io.popen("rm -rf ~/.telegram-cli/data/audio/*") io.popen("rm -rf ~/.telegram-cli/data/document/*") io.popen("rm -rf ~/.telegram-cli/data/photo/*") io.popen("rm -rf ~/.telegram-cli/data/sticker/*") io.popen("rm -rf ~/.telegram-cli/data/temp/*") io.popen("rm -rf ~/.telegram-cli/data/thumb/*") io.popen("rm -rf ~/.telegram-cli/data/video/*") io.popen("rm -rf ~/.telegram-cli/data/voice/*") io.popen("rm -rf ~/.telegram-cli/data/profile_photo/*")   TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*📛┇* تم تحديث البوت', 1, 'md') end 
if text == "وضع اسم البوت ⚡" and is_devtaha(msg) then TIGERBOT:setex(TIGER_ID..'namebot:witting'..msg.sender_user_id_,300,true) TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*📛┇* ارسل لي الاسم 📯\n",1, 'md')  end
if text == 'مسح المميزين عام 🌟' and is_devtaha(msg) then      local list = TIGERBOT:smembers(TIGER_ID..'vip:groups')    if #list == 0 then  TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*📮┇* لا يوجد مميزين عام ليتم مسحهم\n', 1, 'md')   return false  end  local num = 0  for k,v in pairs(list) do    TIGERBOT:srem(TIGER_ID.."vip:groups",v)    num = num + 1  end   TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*☑┇ تم مسح {'..num..'} من المميزين عام *\n', 1, 'md')   end
if text == 'مسح المطورين 👮' and is_devtaha(msg) then     local list = TIGERBOT:smembers(TIGER_ID..'sudo:bot')    if #list == 0 then  TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*📮┇* لا يوجد مطورين ليتم مسحهم\n', 1, 'md')   return false  end  local num = 0  for k,v in pairs(list) do    TIGERBOT:srem(TIGER_ID.."sudo:bot",v)    num = num + 1  end   TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*☑┇ تم مسح {'..num..'} من المطورين *\n', 1, 'md')   end
if text == 'مسح قائمه العام 🚷' and is_devtaha(msg) then   local list = TIGERBOT:smembers(TIGER_ID..'TIGER:gbaned')    if #list == 0 then  TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*📮┇* لا يوجد محظورين عام ليتم مسحهم\n', 1, 'md')   return false  end  local num = 0  for k,v in pairs(list) do    TIGERBOT:srem(TIGER_ID.."TIGER:gbaned",v)    num = num + 1  end   TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*☑┇ تم مسح {'..num..'} من المحظورين عام *\n', 1, 'md')   end
if text and text:match("^ضع رد تواصل 💻$") and is_devtaha(msg) then   TIGERBOT:setex(TIGER_ID.."sudo:pv" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)  TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'📬*┇* ارسل لي النص الذي تريده ', 1, 'md') end 
if text and text:match("^ضع كليشه المطور 🎐$") and is_devtaha(msg) then TIGERBOT:setex(TIGER_ID.."sudo:dev" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)  TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'📬*┇* ارسل لي النص الذي تريده ', 1, 'md') end 
if text and text:match("^ضع كليشه ستارت 📠$") and is_devtaha(msg) then TIGERBOT:setex(TIGER_ID.."start:msgofstart" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)  TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'📬*┇* ارسل لي النص الذي تريده ', 1, 'md') end 
if text == 'تفعيل رساله الترحيب 🎌' and is_devtaha(msg) then local  taha = '*📛┇*تم تفعيل رسالة ترحيب عند الاضافه  ✔' TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") TIGERBOT:del(TIGER_ID..'lock:klsh:add'..bot_id) end 
if text == 'تعطيل رساله الترحيب 📱' and is_devtaha(msg) then taha = '*📮┇*تم تعطيل رسالة ترحيب عند الاضافه  ❌' TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") TIGERBOT:set(TIGER_ID..'lock:klsh:add'..bot_id,true) end
if text == "حذف رد التواصل 📌" and is_devtaha(msg) then  TIGERBOT:del(TIGER_ID.."pvstart") TIGER_sendMsg( msg.chat_id_, msg.id_, 1, '*📛┇*تم حذف رد التوصل', 1, "md") end 
if text == "حذف كليشه ستارت 🃏" and is_devtaha(msg) then  TIGERBOT:del(TIGER_ID.."start:msgofstart1") TIGER_sendMsg( msg.chat_id_, msg.id_, 1, '*📛┇*تم حذف كليشه ستارت', 1, "md") end 
if text == "حذف كليشه المطور 🀄" and is_devtaha(msg) then  TIGERBOT:del(TIGER_ID.."dev") TIGER_sendMsg( msg.chat_id_, msg.id_, 1, '*📛┇*تم حذف كليشه المطور', 1, "md") end 
if text and text:match("^ضع عدد الاعضاء 📜$") and is_devtaha(msg) then  TIGERBOT:setex(TIGER_ID.."numadd:bot" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)  local t = '*📊┇ ارسل لي العدد الان*'  TIGER_sendMsg(msg.chat_id_, msg.id_, 1,t, 1, 'md') end
if text == "جلب رد التواصل 📚" and is_devtaha(msg) then if TIGERBOT:get(TIGER_ID.."pvstart") then pvstart = TIGERBOT:get(TIGER_ID.."pvstart") TIGER_sendMsg(msg.chat_id_, msg.id_, 1,''..check_markdown(pvstart)..'', 1, 'md')  else  TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*✉┇ لا يوجد رد في التواصل \n📮┇* ارسل `ضع رد التواصل`\n✔🙋🏻‍♂️※', 1, 'md')  end  end
if text == "جلب كليشه ستارت 📚" and is_devtaha(msg) then  local start = TIGERBOT:get(TIGER_ID.."start:msgofstart1")  if start then TIGER_sendMsg(msg.chat_id_, msg.id_, 1,''..check_markdown(start)..'', 1, 'md') else TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*🎭┇ لم يتم وضع كليشه ستارت *\n', 1, 'md') end end
if text == 'الاحصائيات 🔭' and is_devtaha(msg) then    local grall = TIGERBOT:scard(TIGER_ID.."botgps") or 0    local gradd = TIGERBOT:scard(TIGER_ID..'bot:gpsby:id') or 0    local uspv = TIGERBOT:scard(TIGER_ID.."usersbot") or 0    TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'\n*📬┇ عدد المجموعات المفعله ↫ ❪'..gradd..'❫\n✔🙋🏻‍♂️※┇ عدد المشتركين ↫ ❪'..uspv..'❫*\n✓※', 1, 'md')   end
if text=="اذاعه بالتوجيه 📬" and msg.reply_to_message_id_ == 0  and is_devtaha(msg) then   TIGERBOT:setex(TIGER_ID.."bc:in:gropsfwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true)   TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*📮┇* ارسل لي التوجيه ليتم اذاعته للمجموعات\n✓※", 1, "md")   end
if text=="اذاعه خاص بالتوجيه 🏷" and msg.reply_to_message_id_ == 0  and is_devtaha(msg) then   TIGERBOT:setex(TIGER_ID.."bc:in:pvfwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true)   TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*📮┇* ارسل لي التوجيه ليتم اذاعته للخاص\n✓※", 1, "md")   end
if text=="اذاعه عام بالتوجيه 💾" and msg.reply_to_message_id_ == 0  and is_devtaha(msg) then   TIGERBOT:setex(TIGER_ID.."bc:in:allfwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true)   TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*📮┇* ارسل لي التوجيه ليتم اذاعته للكل\n✓※", 1, "md")   end
if text=="اذاعه 📡" and msg.reply_to_message_id_ == 0  and is_devtaha(msg) then   TIGERBOT:setex(TIGER_ID.."bc:in:grops" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true)   TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*📮┇* ارسل لي سواء كان » ❪ رساله , صوره , متحركه , ملصق  ❫ للاذاعه الى المجموعات\n✓※", 1, "md")   end
if text=="اذاعه للكل 📡" and msg.reply_to_message_id_ == 0  and is_devtaha(msg) then   TIGERBOT:setex(TIGER_ID.."bc:in:all" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true)   TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*📮┇* ارسل لي سواء كان » ❪ رساله , صوره , متحركه , ملصق  ❫ للاذاعه الى الكل\n✓※", 1, "md")   end
if text=="اذاعه خاص 📡" and msg.reply_to_message_id_ == 0 and is_devtaha(msg) then   TIGERBOT:setex(TIGER_ID.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true)   TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*📮┇* ارسل لي سواء كان » ❪ رساله , صوره , متحركه , ملصق  ❫ للاذاعه الى الخاص\n✓※", 1, "md")   end 
if text ==('المجموعات 📇') and is_devtaha(msg) then    local list = TIGERBOT:smembers(TIGER_ID..'bot:gpsby:id')   if #list == 0 then  TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*✔🙋🏻‍♂️※┇ لا توجد مجموعات حاليا *\n', 1, 'md')  return false  end local t = '⚡┇* اهلا بك في ايدي المجموعات 🍁*\n*ٴ⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃*\n'    for k,v in pairs(list) do     local nummsg = tonumber(TIGERBOT:get(TIGER_ID..'groupmsg:'..v..':'))   numrgroup(v) local numg = (TIGERBOT:get(TIGER_ID.."numgrop"..v) or '3')  local namechat = TIGERBOT:get(TIGER_ID..'group:name'..v)   if namechat then  t = t..'*'..k.."➛* `"..v.."` "..tfgroup(nummsg).."\n*« "..namechat..' » ❪'..numg..'❫*\n*ٴ⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃*\n'     else  t = t..'*'..k.."➛* `"..v.."` "..tfgroup(nummsg).."\n* ❪"..numg..'❫*\n*ٴ⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃*\n'     end  file = io.open("TIGER_groups", "w") file:write([[ ]]..isnothtml(t)..[[ ]]) file:close()   end    t = t..'*📮┇*لعرض معلومات مجموعه معينه\n*✔🙋🏻‍♂️※┇ ارسل كشف من ثم ايدي المجموعه*\n*🚫┇ مثال ❪كشف -10012345667❫*\n꞉'    if #list >= 25 then    local groups = TIGERBOT:scard(TIGER_ID..'bot:gpsby:id')    sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './TIGER_groups','📛┇ عذرا لديك الكثير من المجموعات\n📬┇ تم ارسال المجموعات في الملف\n🚫┇ عدد المجموعات •※'..groups..'※•',dl_cb, nil)   else   TIGER_sendMsg(msg.chat_id_, msg.id_, 1,t, 1, 'md')    end   end
...
os.execute('wget https://raw.githubusercontent.com/TELEFASTR/TIGER/master/TIGER.lua') 
os.execute('cd plugins_;wget https://raw.githubusercontent.com/DEVTELE1/TIGER/master/plugins_/help_rep.lua') 
os.execute('cd plugins_;wget https://raw.githubusercontent.com/DEVTELE1/TIGER/master/plugins_/ZHRAF.lua') 
os.execute('cd plugins_;wget https://raw.githubusercontent.com/DEVTELE1/TIGER/master/plugins_/games.lua') 
sleep(0.5) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '🚸*┇* تم تحديث ♻ السورس ✔ ', 1, 'md') 
dofile('TIGER.lua')  
end

if text == 'المحظورين عام 📑' and is_devtaha(msg) then   local list = TIGERBOT:smembers(TIGER_ID..'TIGER:gbaned')  local t = '*📮┇ قائمه المحظورين عام *\n*ٴ━━━━━━━━━*\n'   for k, v in pairs(list) do   local taha = TIGERBOT:get(TIGER_ID.."user:Name" .. v)  if taha then  local username = taha  t = t..'*'..k.." ➺* ❲["..username.."](tg://user?id="..v..")❳\n"     else  t = t..'*'..k.." ➺* ❲["..v.."](tg://user?id="..v..")❳\n"     end  end   if #list == 0 then   t = '*📬┇* لا يوجد محظورين عام في البوت'   end   TIGER_sendMssg(msg.chat_id_,t,msg.id_,'md')  end  
if text == 'المطورين 📑' and is_devtaha(msg) then   local list = TIGERBOT:smembers(TIGER_ID..'sudo:bot')  local t = '*📮┇ قائمه مطورين البوت *\n*ٴ━━━━━━━━━*\n'   for k, v in pairs(list) do   local taha = TIGERBOT:get(TIGER_ID.."user:Name" .. v)  if taha then  local username = taha  t = t..'*'..k.." ➺* ❲["..username.."](tg://user?id="..v..")❳\n"     else  t = t..'*'..k.." ➺* ❲["..v.."](tg://user?id="..v..")❳\n"     end  end   if #list == 0 then   t = '*📬┇* لا يوجد مطورين في البوت'   end   TIGER_sendMssg(msg.chat_id_,t,msg.id_,'md')  end  
if text == 'المميزين عام 📑' and is_devtaha(msg) then   local list = TIGERBOT:smembers(TIGER_ID..'vip:groups')  local t = '*📮┇ قائمه المميزين عام *\n*ٴ━━━━━━━━━*\n'   for k, v in pairs(list) do   local taha = TIGERBOT:get(TIGER_ID.."user:Name" .. v)  if taha then  local username = taha  t = t..'*'..k.." ➺* ❲["..username.."](tg://user?id="..v..")❳\n"     else  t = t..'*'..k.." ➺* ❲["..v.."](tg://user?id="..v..")❳\n"     end  end   if #list == 0 then   t = '*📬┇* لا يوجد مميزين عام في البوت'   end   TIGER_sendMssg(msg.chat_id_,t,msg.id_,'md')  end  
if text and text:match("^استخراج الرابط 🔦$") and is_devtaha(msg) then   TIGERBOT:setex(TIGER_ID.."get:link:gp" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)   local t = '*📮┇ حسنآ ارسل لي ايدي المجموعه*\n✔🙋🏻‍♂️※'   TIGER_sendMsg(msg.chat_id_, msg.id_, 1,t, 1, 'md')  end
if text and text:match("^كشف 🔍$") and is_devtaha(msg) then   TIGERBOT:setex(TIGER_ID.."get:info:gp" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)  local t = '*📮┇ حسنآ ارسل لي ايدي المجموعه*\n✔🙋🏻‍♂️※'   TIGER_sendMsg(msg.chat_id_, msg.id_, 1,t, 1, 'md')  end
end
if chat_type == 'super' then 
if not TIGERBOT:sismember(TIGER_ID..'bot:gpsby:id',msg.chat_id_) then
...
floods = TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"flood") or 'nil'
NUM_MSG_MAX = TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"floodmax") or 5
TIME_CHECK = TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"floodtime") or 5
if TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"flood") then 
if not is_vipgroup(msg) then
if msg.content_.ID == "MessageChatAddMembers" then 
return else
local post_count = tonumber(TIGERBOT:get(TIGER_ID..'floodc:'..msg.sender_user_id_..':'..msg.chat_id_) or 0)
if post_count > tonumber(TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"floodmax") or 5) then 
local ch = msg.chat_id_
local type = TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"flood") 
trigger_anti_spam(msg,type)  
end
TIGERBOT:setex(TIGER_ID..'floodc:'..msg.sender_user_id_..':'..msg.chat_id_, tonumber(TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"floodtime") or 3), post_count+1) 
end 
end
local edit_id = data.text_ or 'nil'  
NUM_MSG_MAX = 5
if TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"floodmax") then
NUM_MSG_MAX = TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"floodmax") 
end
if TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"floodtime") then
TIME_CHECK = TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"floodtime") 
end 
end	  
if tonumber(TIGERBOT:get('numlockpinmsg'..msg.chat_id_..msg.sender_user_id_) or 1) >= 100  then
TIGERBOT:del(TIGER_ID.."lockpin"..msg.chat_id_) 
else
if msg.content_.ID == 'MessagePinMessage' then    
if is_owner(msg) and TIGERBOT:get(TIGER_ID.."lockpin"..msg.chat_id_) then    
TIGERBOT:set(TIGER_ID..'pinned'..msg.chat_id_, msg.content_.message_id_) 
elseif not TIGERBOT:get(TIGER_ID.."lockpin"..msg.chat_id_) then    
TIGERBOT:set(TIGER_ID..'pinned'..msg.chat_id_, msg.content_.message_id_)    
end    
end
end
...
if TIGERBOT:get(TIGER_ID.."lockpin"..msg.chat_id_) then 
if msg.content_.ID == 'MessagePinMessage' then  
unpinChannelMessage(msg.chat_id_)  
local PinnedMessage = TIGERBOT:get(TIGER_ID..'pinned'..msg.chat_id_)  
...
if TIGERBOT:get(TIGER_ID..'lock:text'..msg.chat_id_) then       
delete_msg(msg.chat_id_,{[0] = msg.id_})   
return false end    
end     
---الاشعارات
if msg.content_.ID == "MessageChatDeletePhoto" or msg.content_.ID == "MessageChatChangePhoto" or msg.content_.ID == 'MessagePinMessage' or msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" or msg.content_.ID == 'MessageChatChangeTitle' or msg.content_.ID == "MessageChatDeleteMember" then   
if TIGERBOT:get(TIGER_ID..'lock:tagservr'..msg.chat_id_) then  
delete_msg(msg.chat_id_,{[0] = msg.id_})       
end    
end   
---الاضافات والدخول
if msg.content_.ID == "MessageChatAddMembers" and not is_vipgroup(msg) then   
if TIGERBOT:get(TIGER_ID.."lock:AddMempar"..msg.chat_id_) == 'kick' then
local mem_id = msg.content_.members_  
for i=0,#mem_id do  
...
if TIGERBOT:get(TIGER_ID.."lock:Join"..msg.chat_id_) == 'kick' then
...
if TIGERBOT:get(TIGER_ID.."lock:user:name"..msg.chat_id_) == "del" and not is_vipgroup(msg) then    
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:user:name"..msg.chat_id_) == "ked" and not is_vipgroup(msg) then    
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:user:name"..msg.chat_id_) == "kick" and not is_vipgroup(msg) then    
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:user:name"..msg.chat_id_) == "ktm" and not is_vipgroup(msg) then    
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if msg.content_.caption_ then 
if msg.content_.caption_:match("#[%a%d_]+") or msg.content_.caption_:match("#(.+)") then 
if TIGERBOT:get(TIGER_ID.."lock:hashtak"..msg.chat_id_) == "del" and not is_vipgroup(msg) then    
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:hashtak"..msg.chat_id_) == "ked" and not is_vipgroup(msg) then    
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:hashtak"..msg.chat_id_) == "kick" and not is_vipgroup(msg) then    
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:hashtak"..msg.chat_id_) == "ktm" and not is_vipgroup(msg) then    
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if msg.content_.caption_ then 
if msg.content_.caption_:match("/[%a%d_]+") or msg.content_.caption_:match("/(.+)") then  
if TIGERBOT:get(TIGER_ID.."lock:Cmd"..msg.chat_id_) == "del" and not is_vipgroup(msg) then    
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Cmd"..msg.chat_id_) == "ked" and not is_vipgroup(msg) then    
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Cmd"..msg.chat_id_) == "kick" and not is_vipgroup(msg) then    
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Cmd"..msg.chat_id_) == "ktm" and not is_vipgroup(msg) then    
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if TIGERBOT:get(TIGER_ID.."lock:Link"..msg.chat_id_) == "del" and not is_vipgroup(msg) then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Link"..msg.chat_id_) == "ked" and not is_vipgroup(msg) then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Link"..msg.chat_id_) == "kick" and not is_vipgroup(msg) then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Link"..msg.chat_id_) == "ktm" and not is_vipgroup(msg) then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if TIGERBOT:get(TIGER_ID.."lock:Photo"..msg.chat_id_) == "del" then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Photo"..msg.chat_id_) == "ked" then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Photo"..msg.chat_id_) == "kick" then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Photo"..msg.chat_id_) == "ktm" then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if TIGERBOT:get(TIGER_ID.."lock:Video"..msg.chat_id_) == "del" then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Video"..msg.chat_id_) == "ked" then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Video"..msg.chat_id_) == "kick" then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Video"..msg.chat_id_) == "ktm" then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if TIGERBOT:get(TIGER_ID.."lock:Animation"..msg.chat_id_) == "del" then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Animation"..msg.chat_id_) == "ked" then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Animation"..msg.chat_id_) == "kick" then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Animation"..msg.chat_id_) == "ktm" then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if TIGERBOT:get(TIGER_ID.."lock:geam"..msg.chat_id_) == "del" then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:geam"..msg.chat_id_) == "ked" then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:geam"..msg.chat_id_) == "kick" then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:geam"..msg.chat_id_) == "ktm" then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if TIGERBOT:get(TIGER_ID.."lock:Audio"..msg.chat_id_) == "del" then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Audio"..msg.chat_id_) == "ked" then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Audio"..msg.chat_id_) == "kick" then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Audio"..msg.chat_id_) == "ktm" then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if TIGERBOT:get(TIGER_ID.."lock:vico"..msg.chat_id_) == "del" then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:vico"..msg.chat_id_) == "ked" then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:vico"..msg.chat_id_) == "kick" then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:vico"..msg.chat_id_) == "ktm" then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if TIGERBOT:get(TIGER_ID.."lock:Keyboard"..msg.chat_id_) == "del" then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Keyboard"..msg.chat_id_) == "ked" then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Keyboard"..msg.chat_id_) == "kick" then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Keyboard"..msg.chat_id_) == "ktm" then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if TIGERBOT:get(TIGER_ID.."lock:Sticker"..msg.chat_id_) == "del" then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Sticker"..msg.chat_id_) == "ked" then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Sticker"..msg.chat_id_) == "kick" then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Sticker"..msg.chat_id_) == "ktm" then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if TIGERBOT:get(TIGER_ID.."lock:forward"..msg.chat_id_) == "del" then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
return false
elseif TIGERBOT:get(TIGER_ID.."lock:forward"..msg.chat_id_) == "ked" then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
return false
elseif TIGERBOT:get(TIGER_ID.."lock:forward"..msg.chat_id_) == "kick" then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
return false
elseif TIGERBOT:get(TIGER_ID.."lock:forward"..msg.chat_id_) == "ktm" then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
return false
end
...
if TIGERBOT:get(TIGER_ID.."lock:Document"..msg.chat_id_) == "del" then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Document"..msg.chat_id_) == "ked" then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Document"..msg.chat_id_) == "kick" then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Document"..msg.chat_id_) == "ktm" then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if TIGERBOT:get(TIGER_ID.."lock:Unsupported"..msg.chat_id_) == "del" then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Unsupported"..msg.chat_id_) == "ked" then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Unsupported"..msg.chat_id_) == "kick" then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Unsupported"..msg.chat_id_) == "ktm" then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if not is_vipgroup(msg) then
if TIGERBOT:get(TIGER_ID.."lock:Markdaun"..msg.chat_id_) == "del" then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Markdaun"..msg.chat_id_) == "ked" then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Markdaun"..msg.chat_id_) == "kick" then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Markdaun"..msg.chat_id_) == "ktm" then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end  
...
if TIGERBOT:get(TIGER_ID.."lock:Contact"..msg.chat_id_) == "del" then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Contact"..msg.chat_id_) == "ked" then
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Contact"..msg.chat_id_) == "kick" then
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Contact"..msg.chat_id_) == "ktm" then
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
sens = 400  
if TIGERBOT:get(TIGER_ID.."lock:Spam"..msg.chat_id_) == "del" and utf8.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Spam"..msg.chat_id_) == "ked" and utf8.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
ked(msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Spam"..msg.chat_id_) == "kick" and utf8.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
kicck(msg,msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
elseif TIGERBOT:get(TIGER_ID.."lock:Spam"..msg.chat_id_) == "ktm" and utf8.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
TIGERBOT:sadd(TIGER_ID..'mutes'..msg.chat_id_,msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if msg.content_.ID == 'MessagePinMessage' then
if TIGERBOT:sismember(TIGER_ID..'LOCK:PINMSG'..msg.chat_id_,msg.sender_user_id_) and not is_owner(msg) then 
local PinnedMessage = TIGERBOT:get(TIGER_ID..'pinned'..msg.chat_id_)  
if PinnedMessage then  
pinChannelMessage(msg.chat_id_,tonumber(PinnedMessage),0) 
end
...
if msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/") or msg.content_.caption_:match("[Hh][Tt][Tt][Pp][Ss]://") or msg.content_.caption_:match("[Hh][Tt][Tt][Pp]://") or msg.content_.caption_:match("[Ww][Ww][Ww].") or msg.content_.caption_:match(".[Cc][Oo][Mm]") or msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/") or msg.content_.caption_:match(".[Pp][Ee]") or msg.content_.caption_:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]/") or msg.content_.caption_:match("[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/") or msg.content_.caption_:match("[Tt].[Mm][Ee]/") then 
if TIGERBOT:sismember(TIGER_ID..'LOCK:LINKS'..msg.chat_id_,msg.sender_user_id_) then
tdcli_function ({ID="DeleteMessages", chat_id_=msg.chat_id_, message_ids_={[0] = msg.id_}}, dl_cb, nil) 
end
end
end
if text and text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/") or text and text:match("[Hh][Tt][Tt][Pp][Ss]://") or text and text:match("[Hh][Tt][Tt][Pp]://") or text and text:match("[Ww][Ww][Ww].") or text and text:match(".[Cc][Oo][Mm]") or text and text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/") or text and text:match(".[Pp][Ee]") or text and text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]/") or text and text:match("[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/") or text and text:match("[Tt].[Mm][Ee]/") and not is_vipgroup(msg) then
if TIGERBOT:sismember(TIGER_ID..'LOCK:LINKS'..msg.chat_id_,msg.sender_user_id_) and not is_owner(msg) then 
tdcli_function ({ID="DeleteMessages", chat_id_=msg.chat_id_, message_ids_={[0] = msg.id_}}, dl_cb, nil) 
end
end
-- المعرفات
if text and text:match("@[%a%d_]+") or text and text:match("@(.+)") and not is_owner(msg) then     
if TIGERBOT:sismember(TIGER_ID..'LOCK:USERNAME'..msg.chat_id_,msg.sender_user_id_) then
tdcli_function ({ID="DeleteMessages", chat_id_=msg.chat_id_, message_ids_={[0] = msg.id_}}, dl_cb, nil) 
end
end
if msg.content_.caption_ and not is_owner(msg) then  
...
-- الصور
if msg.content_.ID == 'MessagePhoto' then 
if TIGERBOT:sismember(TIGER_ID..'LOCK:PHOTO'..msg.chat_id_,msg.sender_user_id_) and not is_owner(msg) then 
tdcli_function ({ID="DeleteMessages", chat_id_=msg.chat_id_, message_ids_={[0] = msg.id_}}, dl_cb, nil) 
end
end
-- الصوت
if msg.content_.ID == 'MessageVoice' or msg.content_.ID == 'MessageAudio' then
if TIGERBOT:sismember(TIGER_ID..'LOCK:VICO'..msg.chat_id_,msg.sender_user_id_) and not is_owner(msg) then 
tdcli_function ({ID="DeleteMessages", chat_id_=msg.chat_id_, message_ids_={[0] = msg.id_}}, dl_cb, nil) 
end
end
...
--المتحركه
if msg.content_.ID == 'MessageAnimation' then
if TIGERBOT:sismember(TIGER_ID..'LOCK:GIF'..msg.chat_id_,msg.sender_user_id_) and not is_owner(msg) then 
tdcli_function ({ID="DeleteMessages", chat_id_=msg.chat_id_, message_ids_={[0] = msg.id_}}, dl_cb, nil) 
end
end
--الفيديو
if msg.content_.ID == 'MessageVideo' then
if TIGERBOT:sismember(TIGER_ID..'LOCK:VIDEO'..msg.chat_id_,msg.sender_user_id_) and not is_owner(msg) then 
tdcli_function ({ID="DeleteMessages", chat_id_=msg.chat_id_, message_ids_={[0] = msg.id_}}, dl_cb, nil) 
end
end
--الملصقات
if msg.content_.ID == 'MessageSticker' then     
print('ملصق')
if TIGERBOT:sismember(TIGER_ID..'LOCK:STEKR'..msg.chat_id_,msg.sender_user_id_) and not is_owner(msg) then 
tdcli_function ({ID="DeleteMessages", chat_id_=msg.chat_id_, message_ids_={[0] = msg.id_}}, dl_cb, nil) 
end
end
--السيبفي
if msg.content_.ID == "MessageUnsupported" then
if TIGERBOT:sismember(TIGER_ID..'LOCK:SELPHY'..msg.chat_id_,msg.sender_user_id_) and not is_owner(msg) then 
tdcli_function ({ID="DeleteMessages", chat_id_=msg.chat_id_, message_ids_={[0] = msg.id_}}, dl_cb, nil) 
end
end
--الماركدون
...
if msg.content_.entities_[0] then 
if msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityUrl" or msg.content_.entities_[0].ID == "MessageEntityTextUrl" then      
if TIGERBOT:sismember(TIGER_ID..'LOCK:MARKDWN'..msg.chat_id_,msg.sender_user_id_) and not is_owner(msg) then 
tdcli_function ({ID="DeleteMessages", chat_id_=msg.chat_id_, message_ids_={[0] = msg.id_}}, dl_cb, nil) 
end
end
end
...
--التوجيه
if msg.forward_info_ then
if TIGERBOT:sismember(TIGER_ID..'LOCK:FWD'..msg.chat_id_,msg.sender_user_id_) and not is_owner(msg) then 
tdcli_function ({ID="DeleteMessages", chat_id_=msg.chat_id_, message_ids_={[0] = msg.id_}}, dl_cb, nil) 
end
end
--الاونلاين
if msg.reply_markup_ and msg.reply_markup_.ID == 'ReplyMarkupInlineKeyboard' then     
if TIGERBOT:sismember(TIGER_ID..'LOCK:INLIN'..msg.chat_id_,msg.sender_user_id_) and not is_owner(msg) then 
tdcli_function ({ID="DeleteMessages", chat_id_=msg.chat_id_, message_ids_={[0] = msg.id_}}, dl_cb, nil) 
end
end 
 
local status_welcome = (TIGERBOT:get(TIGER_ID..'add:welc:'..msg.chat_id_) or 'rem')  
if status_welcome == 'add' and not TIGERBOT:get(TIGER_ID..'lock:tagservr'..msg.chat_id_)  then
if msg.content_.ID == "MessageChatJoinByLink" then
if not is_banned(msg.chat_id_,msg.sender_user_id_) then 
function wlc(extra,result,success) 
if TIGERBOT:get(TIGER_ID..'welcome:'..msg.chat_id_) then 
t = TIGERBOT:get(TIGER_ID..'welcome:'..msg.chat_id_) 
else  
t = '\n• نورت حبي \n•  name \n• ngp' 
end 
t = t:gsub('name','<TIGER>'..CatchName(result.first_name_,25)..'</TIGER>') 
t = t:gsub('ngp',TIGERBOT:get(TIGER_ID..'group:name'..msg.chat_id_)) 
...
TIGERBOT:setex(TIGER_ID.."photo:bot" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)  
local t = '*💬┇* ارسل رقم الصوره الان '  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,t, 1, 'md') 
end
if text and text == 'تغير كليشه الترحيب' and is_devtaha(msg)  then    
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*💬┇ حسنآ ارسل لي نص الترحيب *\n', 1, 'md')   
TIGERBOT:set("addreply1:"..msg.sender_user_id_..bot_id,"rep")   
return false   end     
if text then    
local rep = TIGERBOT:get("addreply1:"..msg.sender_user_id_..bot_id)   
if rep == 'rep' then    
if text and text:match("^الغاء$") then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*☑┇ تم الغاء الامر *\n✓※", 1, "md") 
TIGERBOT:del("addreply1:"..msg.sender_user_id_..bot_id)   
return false  end 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*💬┇ ارسل لي صورة الترحيب *\n", 1, 'md')   
TIGERBOT:set("addreply1:"..msg.sender_user_id_..bot_id,"repp")   
TIGERBOT:set("addreply2:"..msg.sender_user_id_..bot_id, text)   
TIGERBOT:set("klish:welc"..bot_id,text)   
return false   
end   
end   
...
TIGERBOT:set("addreply1photo1"..bot_id, msg.content_.photo_.sizes_[1].photo_.persistent_id_)   
end 
end   
TIGERBOT:del("addreply2:"..msg.sender_user_id_..bot_id)   
return false   end   
end
--======================
--ردود المجموعه بالرد
...
local taha = TIGERBOT:scard(TIGER_ID.."rep:media"..msg.chat_id_)  
for k,v in pairs(list) do  
TIGERBOT:del(TIGER_ID.."addreply1:"..msg.sender_user_id_..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."addreply1:gif"..v..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."addreply1:vico"..v..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."addreply1:stekr"..v..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."taha:"..v..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."addreply1:photo"..v..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."addreply1:video"..v..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."addreply1:document"..v..msg.chat_id_)  
...
local list = TIGERBOT:smembers(TIGER_ID.."rep:media"..msg.chat_id_)  
t = "*📜┇ قائمة ردود المجموعه بالرد ✔🙋🏻‍♂️※\nٴ━━━━━━━━━━━*\n"    
for k,v in pairs(list) do  
if TIGERBOT:get(TIGER_ID.."addreply1:gif"..v..msg.chat_id_) then
taha = 'متحركه 🎆'
elseif TIGERBOT:get(TIGER_ID.."addreply1:vico"..v..msg.chat_id_)  then
taha = 'بصمه 🎵'
elseif TIGERBOT:get(TIGER_ID.."addreply1:stekr"..v..msg.chat_id_)  then
taha = 'ملصق 🃏'
elseif TIGERBOT:get(TIGER_ID.."taha:"..v..msg.chat_id_) then
taha = 'رساله 💭'
elseif TIGERBOT:get(TIGER_ID.."addreply1:photo"..v..msg.chat_id_)  then
taha = 'صوره 🌇'
elseif TIGERBOT:get(TIGER_ID.."addreply1:video"..v..msg.chat_id_)  then
taha = 'فيديو 📹'
elseif TIGERBOT:get(TIGER_ID.."addreply1:document"..v..msg.chat_id_)  then
taha = 'ملف 📁'
elseif TIGERBOT:get(TIGER_ID.."addreply1:audio"..v..msg.chat_id_)  then
taha = 'اغنيه 🎵'
end
t = t..'*'..k..'• *❨`'..v..'`❩ *» {'..taha..'}*\n'    
end  
...
TIGERBOT:set(TIGER_ID.."addreply1:"..msg.sender_user_id_..msg.chat_id_,"rep")  
return false  end    
if text then   
local tsssst = TIGERBOT:get(TIGER_ID.."addreply1:"..msg.sender_user_id_..msg.chat_id_)  
if tsssst == 'rep' then   
if text and text:match("^الغاء$") then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*☑┇ تم الغاء الامر *\n✓※", 1, "md") 
TIGERBOT:del(TIGER_ID.."addreply1:"..msg.sender_user_id_..msg.chat_id_)  
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*💬┇ حسنآ ارسل لي الكلمه الان *\n✔🙋🏻‍♂️※', 1, 'md')  
TIGERBOT:set(TIGER_ID.."addreply1:"..msg.sender_user_id_..msg.chat_id_,"reppp")  
return false  end
if text then 
local test = TIGERBOT:get(TIGER_ID.."addreply1:"..msg.sender_user_id_..msg.chat_id_)  
...
TIGERBOT:del("taha:"..text..msg.chat_id_)  
TIGERBOT:del("addreply1:photo"..text..msg.chat_id_)
TIGERBOT:del("addreply1:video"..text..msg.chat_id_)
TIGERBOT:del("addreply1:document"..text..msg.chat_id_)
...
TIGERBOT:del(TIGER_ID.."addreply1:stekr"..text..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."taha:"..text..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."addreply1:photo"..text..msg.chat_id_)
TIGERBOT:del(TIGER_ID.."addreply1:video"..text..msg.chat_id_)
TIGERBOT:del(TIGER_ID.."addreply1:document"..text..msg.chat_id_)
TIGERBOT:del(TIGER_ID.."addreply1:audio"..text..msg.chat_id_)
TIGERBOT:srem(TIGER_ID.."rep:media"..msg.chat_id_,text)  
end
return false  end  
end
...
TIGERBOT:set(TIGER_ID.."addreply1:stekr"..test..msg.chat_id_, msg.content_.sticker_.sticker_.persistent_id_)  
end   
if msg.content_.voice_ then  
TIGERBOT:set(TIGER_ID.."addreply1:vico"..test..msg.chat_id_, msg.content_.voice_.voice_.persistent_id_)  
end   
if msg.content_.animation_ then   
TIGERBOT:set(TIGER_ID.."addreply1:gif"..test..msg.chat_id_, msg.content_.animation_.animation_.persistent_id_)  
end  
if text then   
TIGERBOT:set(TIGER_ID.."taha:"..test..msg.chat_id_, text)  
end  
if msg.content_.audio_ then
TIGERBOT:set(TIGER_ID.."addreply1:audio"..test..msg.chat_id_, msg.content_.audio_.audio_.persistent_id_)  
end
if msg.content_.document_ then
TIGERBOT:set(TIGER_ID.."addreply1:document"..test..msg.chat_id_, msg.content_.document_.document_.persistent_id_)  
end
if msg.content_.video_ then
TIGERBOT:set(TIGER_ID.."addreply1:video"..test..msg.chat_id_, msg.content_.video_.video_.persistent_id_)  
TIGERBOT:set(TIGER_ID.."addreply1:video:caption"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
...
TIGERBOT:set(TIGER_ID.."addreply1:photo"..test..msg.chat_id_, photo_in_group)  
TIGERBOT:set(TIGER_ID.."addreply1:photo:caption"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
TIGERBOT:del(TIGER_ID.."addreply2:"..msg.sender_user_id_..msg.chat_id_)  
return false  end  
end

if text and msg.reply_to_message_id_ ~= 0 and not TIGERBOT:get(TIGER_ID..'lock:rep:rd'..msg.chat_id_) then  
local anemi = TIGERBOT:get(TIGER_ID.."addreply1:gif"..text..msg.chat_id_)   
local veico = TIGERBOT:get(TIGER_ID.."addreply1:vico"..text..msg.chat_id_)   
local stekr = TIGERBOT:get(TIGER_ID.."addreply1:stekr"..text..msg.chat_id_)     
local taha = TIGERBOT:get(TIGER_ID.."taha:"..text..msg.chat_id_)   
local photo = TIGERBOT:get(TIGER_ID.."addreply1:photo"..text..msg.chat_id_)
local photo_caption = (TIGERBOT:get(TIGER_ID.."addreply1:photo:caption"..text..msg.chat_id_) or '' )
local video = TIGERBOT:get(TIGER_ID.."addreply1:video"..text..msg.chat_id_)
local video_caption = TIGERBOT:get(TIGER_ID.."addreply1:video:caption"..text..msg.chat_id_)
local document = TIGERBOT:get(TIGER_ID.."addreply1:document"..text..msg.chat_id_)
local audio = TIGERBOT:get(TIGER_ID.."addreply1:audio"..text..msg.chat_id_)
if taha then    
TIGER_sendMsg(msg.chat_id_, msg.reply_to_message_id_, 1, ''..check_markdown(taha)..'', 1, 'md')     
return false   
end    
if veico then    
...
local taha = TIGERBOT:scard(TIGER_ID.."repmedia"..msg.chat_id_)  
for k,v in pairs(list) do  
TIGERBOT:del(TIGER_ID.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."add:reply1:gif"..v..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."add:reply1:vico"..v..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."add:reply1:stekr"..v..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."add:reply:rd"..v..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."addreply1:photo:gp"..v..msg.chat_id_)
TIGERBOT:del(TIGER_ID.."addreply1:video:gp"..v..msg.chat_id_)
TIGERBOT:del(TIGER_ID.."addreply1:document:gp"..v..msg.chat_id_)
...
local list = TIGERBOT:smembers(TIGER_ID.."repmedia"..msg.chat_id_)  
t = "*📜┇ قائمة ردود المجموعه ✔🙋🏻‍♂️※\nٴ━━━━━━━━━━━*\n"    
for k,v in pairs(list) do  
if TIGERBOT:get(TIGER_ID.."add:reply1:gif"..v..msg.chat_id_) then
taha = 'متحركه 🎆'
elseif TIGERBOT:get(TIGER_ID.."add:reply1:vico"..v..msg.chat_id_) then
taha = 'بصمه 🎙'
elseif TIGERBOT:get(TIGER_ID.."add:reply1:stekr"..v..msg.chat_id_) then
taha = 'ملصق 🃏'
elseif TIGERBOT:get(TIGER_ID.."add:reply:rd"..v..msg.chat_id_) then
taha = 'رساله 💭'
elseif TIGERBOT:get(TIGER_ID.."addreply1:photo:gp"..v..msg.chat_id_) then
taha = 'صوره 🌇'
elseif TIGERBOT:get(TIGER_ID.."addreply1:video:gp"..v..msg.chat_id_) then
taha = 'فيديو 📹'
elseif TIGERBOT:get(TIGER_ID.."addreply1:document:gp"..v..msg.chat_id_) then
taha = 'ملف 📁'
elseif TIGERBOT:get(TIGER_ID.."addreply1:audio:gp"..v..msg.chat_id_) then
taha = 'اغنيه 🎵'
end
t = t..'*'..k..'• *❨`'..v..'`❩ *» {'..taha..'}*\n'    
end  
...
if text then   
local tsssst = TIGERBOT:get(TIGER_ID.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if tsssst == 'rep' then   
if text and text:match("^الغاء$") then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*☑┇ تم الغاء الامر *\n✓※", 1, "md") 
TIGERBOT:del(TIGER_ID.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
return false  end 
if TIGERBOT:sismember(TIGER_ID..'repmedia'..msg.chat_id_,text) then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*☑┇ لقد تم اضافة رد بهاذه الكلمه \n💬┇ ارسل كلمه اخرى او ارسل الغاء*\n✔🙋🏻‍♂️※\n", 1, 'md')  
else
media = '{ متحركه ‹› ملصق ‹› صوره ‹› اغنيه ‹› بصمه ‹› ملف ‹› فيديو }'
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*💬┇ حسنآ ارسل الرد الان\n🔖┇ يمكنك ارسال الرد » "..media.."*\n✔🙋🏻‍♂️※", 1, 'md')  
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*💬┇ حسنآ ارسل لي الكلمه الان *\n', 1, 'md')  
TIGERBOT:set(TIGER_ID.."add:reply1"..msg.sender_user_id_..msg.chat_id_,"reppp")  
return false  end
if text then 
local test = TIGERBOT:get(TIGER_ID.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if test and test == 'reppp' then   
...
TIGERBOT:del("add:reply:rd"..text..msg.chat_id_)  
TIGERBOT:del("addreply1:photo:gp"..text..msg.chat_id_)
TIGERBOT:del("addreply1:video:gp"..text..msg.chat_id_)
TIGERBOT:del("addreply1:document:gp"..text..msg.chat_id_)
...
TIGERBOT:del(TIGER_ID.."add:reply1:stekr"..text..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."add:reply:rd"..text..msg.chat_id_)  
TIGERBOT:del(TIGER_ID.."addreply1:photo:gp"..text..msg.chat_id_)
TIGERBOT:del(TIGER_ID.."addreply1:video:gp"..text..msg.chat_id_)
TIGERBOT:del(TIGER_ID.."addreply1:document:gp"..text..msg.chat_id_)
TIGERBOT:del(TIGER_ID.."addreply1:audio:gp"..text..msg.chat_id_)
TIGERBOT:srem(TIGER_ID.."repmedia"..msg.chat_id_,text)  
end
return false  end  
end
...
if msg.content_.sticker_ then   
TIGERBOT:set(TIGER_ID.."add:reply1:stekr"..test..msg.chat_id_, msg.content_.sticker_.sticker_.persistent_id_)  
end   
if msg.content_.voice_ then  
TIGERBOT:set(TIGER_ID.."add:reply1:vico"..test..msg.chat_id_, msg.content_.voice_.voice_.persistent_id_)  
end   
if msg.content_.animation_ then   
TIGERBOT:set(TIGER_ID.."add:reply1:gif"..test..msg.chat_id_, msg.content_.animation_.animation_.persistent_id_)  
end  
if text then   
TIGERBOT:set(TIGER_ID.."add:reply:rd"..test..msg.chat_id_, text)  
end  
if msg.content_.audio_ then
TIGERBOT:set(TIGER_ID.."addreply1:audio:gp"..test..msg.chat_id_, msg.content_.audio_.audio_.persistent_id_)  
end
if msg.content_.document_ then
TIGERBOT:set(TIGER_ID.."addreply1:document:gp"..test..msg.chat_id_, msg.content_.document_.document_.persistent_id_)  
end
if msg.content_.video_ then
TIGERBOT:set(TIGER_ID.."addreply1:video:gp"..test..msg.chat_id_, msg.content_.video_.video_.persistent_id_)  
TIGERBOT:set(TIGER_ID.."addreply1:video:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo_in_group = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
TIGERBOT:set(TIGER_ID.."addreply1:photo:gp"..test..msg.chat_id_, photo_in_group)  
TIGERBOT:set(TIGER_ID.."addreply1:photo:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
TIGERBOT:del(TIGER_ID.."add:reply2"..msg.sender_user_id_..msg.chat_id_)  
return false  end  
end
if text and not TIGERBOT:get(TIGER_ID..'lock:rep'..msg.chat_id_) then  
local anemi = TIGERBOT:get(TIGER_ID.."add:reply1:gif"..text..msg.chat_id_)   
local veico = TIGERBOT:get(TIGER_ID.."add:reply1:vico"..text..msg.chat_id_)   
local stekr = TIGERBOT:get(TIGER_ID.."add:reply1:stekr"..text..msg.chat_id_)     
local taha = TIGERBOT:get(TIGER_ID.."add:reply:rd"..text..msg.chat_id_)   
local video_caption = TIGERBOT:get(TIGER_ID.."addreply1:video:caption:gp"..text..msg.chat_id_)
local photo_caption = (TIGERBOT:get(TIGER_ID.."addreply1:photo:caption:gp"..text..msg.chat_id_) or '' )
local photo = TIGERBOT:get(TIGER_ID.."addreply1:photo:gp"..text..msg.chat_id_)
local video = TIGERBOT:get(TIGER_ID.."addreply1:video:gp"..text..msg.chat_id_)
local document = TIGERBOT:get(TIGER_ID.."addreply1:document:gp"..text..msg.chat_id_)
local audio = TIGERBOT:get(TIGER_ID.."addreply1:audio:gp"..text..msg.chat_id_)
if taha then    
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, ''..check_markdown(taha)..'', 1, 'md')     
return false   
end    
if veico then    
...
if TIGERBOT:get(TIGER_ID..'lock:rep:all:rd'..bot_id) then
taha = '*☑┇ تم تفعيل ردود المطور بالرد *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:del(TIGER_ID..'lock:rep:all:rd'..bot_id)   
else
...
if not TIGERBOT:get(TIGER_ID..'lock:rep:all:rd'..bot_id) then
taha = '*☑┇ تم تعطيل ردود المطور بالرد *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:set(TIGER_ID..'lock:rep:all:rd'..bot_id,true)   
else
...
local taha = TIGERBOT:scard(TIGER_ID.."rep:media:all:rd"..bot_id)    
for k,v in pairs(list) do    
TIGERBOT:del(TIGER_ID.."add:repallt:rd"..msg.sender_user_id_..bot_id)    
TIGERBOT:del(TIGER_ID.."add:repallt:gif:all:rd"..v..bot_id)    
TIGERBOT:del(TIGER_ID.."add:rep:tvico:all:rd"..v..bot_id)    
TIGERBOT:del(TIGER_ID.."add:rep:tstekr:all:rd"..v..bot_id)    
TIGERBOT:del(TIGER_ID.."add:rep:text:all:rd"..v..bot_id)    
TIGERBOT:del(TIGER_ID.."all:addreply1:photo:gp"..v..bot_id)
TIGERBOT:del(TIGER_ID.."all:addreply1:video:gp"..v..bot_id)
TIGERBOT:del(TIGER_ID.."all:addreply1:document:gp"..v..bot_id)
...
local list = TIGERBOT:smembers(TIGER_ID.."rep:media:all:rd"..bot_id)    
t = "*📜┇ قائمة ردود المطور بالرد ✔🙋🏻‍♂️※\nٴ━━━━━━━━━━━*\n"    
for k,v in pairs(list) do    
if TIGERBOT:get(TIGER_ID.."add:repallt:gif:all:rd"..v..bot_id) then
taha = 'متحركه 🎆'
elseif TIGERBOT:get(TIGER_ID.."add:rep:tvico:all:rd"..v..bot_id) then
taha = 'بصمه 🎙'
elseif TIGERBOT:get(TIGER_ID.."add:rep:tstekr:all:rd"..v..bot_id)  then
taha = 'ملصق 🃏'
elseif TIGERBOT:get(TIGER_ID.."add:rep:text:all:rd"..v..bot_id) then
taha = 'رساله 💭'
elseif TIGERBOT:get(TIGER_ID.."all:addreply1:photo:gp"..v..bot_id)  then
taha = 'صوره 🌇'
elseif TIGERBOT:get(TIGER_ID.."all:addreply1:video:gp"..v..bot_id)  then
taha = 'فيديو 📹'
elseif TIGERBOT:get(TIGER_ID.."all:addreply1:document:gp"..v..bot_id)  then
taha = 'ملف 📁'
elseif TIGERBOT:get(TIGER_ID.."all:addreply1:audio:gp"..v..bot_id)  then
taha = 'اغنيه 🎵'
end
t = t..'*'..k..'• *❨`'..v..'`❩ *» {'..taha..'}*\n'    
end    
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*💬┇ حسنآ ارسل لي الكلمه الان *\n✔🙋🏻‍♂️※', 1, 'md')  
TIGERBOT:set(TIGER_ID.."add:repallt:rd"..msg.sender_user_id_..bot_id,'yes')    
return false    end      
if text then     
local tt = TIGERBOT:get(TIGER_ID.."add:repallt:rd"..msg.sender_user_id_..bot_id)    
if tt == 'yes' then     
if text and text:match("^الغاء$") then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*☑┇ تم الغاء الامر *\n✓※", 1, "md") 
TIGERBOT:del(TIGER_ID.."add:repallt:rd"..msg.sender_user_id_..bot_id)    
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*💬┇ حسنآ ارسل لي الكلمه الان *\n✔🙋🏻‍♂️※', 1, 'md')  
TIGERBOT:set(TIGER_ID.."add:repallt:rd"..msg.sender_user_id_..bot_id,'yes11')    
return false    end    
--للكل بالرد
if text then 
local test = TIGERBOT:get(TIGER_ID.."add:repallt:rd"..msg.sender_user_id_..bot_id)    
...
TIGERBOT:del("add:rep:text:all:rd"..text..bot_id)    
TIGERBOT:del("all:addreply1:photo:gp"..text..bot_id)
TIGERBOT:del("all:addreply1:video:gp"..text..bot_id)
TIGERBOT:del("all:addreply1:document:gp"..text..bot_id)
...
TIGERBOT:del(TIGER_ID.."add:rep:tstekr:all:rd"..text..bot_id)    
TIGERBOT:del(TIGER_ID.."add:rep:text:all:rd"..text..bot_id)    
TIGERBOT:del(TIGER_ID.."all:addreply1:photo:gp"..text..bot_id)
TIGERBOT:del(TIGER_ID.."all:addreply1:video:gp"..text..bot_id)
TIGERBOT:del(TIGER_ID.."all:addreply1:document:gp"..text..bot_id)
TIGERBOT:del(TIGER_ID.."all:addreply1:audio:gp"..text..bot_id)
TIGERBOT:srem(TIGER_ID.."rep:media:all:rd"..bot_id,text)    
end
return false    end    
end
if text or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.animation_ or msg.content_.audio_ or msg.content_.document_ or msg.content_.photo_ or msg.content_.video_ then  
...
if msg.content_.sticker_ then     
TIGERBOT:set(TIGER_ID.."add:rep:tstekr:all:rd"..test..bot_id, msg.content_.sticker_.sticker_.persistent_id_)    
end     
if msg.content_.voice_ then    
TIGERBOT:set(TIGER_ID.."add:rep:tvico:all:rd"..test..bot_id, msg.content_.voice_.voice_.persistent_id_)    
end     
if msg.content_.animation_ then     
TIGERBOT:set(TIGER_ID.."add:repallt:gif:all:rd"..test..bot_id, msg.content_.animation_.animation_.persistent_id_)    
end    
if text then     
TIGERBOT:set(TIGER_ID.."add:rep:text:all:rd"..test..bot_id, text)    
end    
if msg.content_.audio_ then
TIGERBOT:set(TIGER_ID.."all:addreply1:audio:gp"..test..bot_id, msg.content_.audio_.audio_.persistent_id_)  
end
if msg.content_.document_ then
TIGERBOT:set(TIGER_ID.."all:addreply1:document:gp"..test..bot_id, msg.content_.document_.document_.persistent_id_)  
end
if msg.content_.video_ then
TIGERBOT:set(TIGER_ID.."all:addreply1:video:gp"..test..bot_id, msg.content_.video_.video_.persistent_id_)  
TIGERBOT:set(TIGER_ID.."all:addreply1:video:caption:gp"..test..bot_id,(msg.content_.caption_ or ''))  
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo_in_group = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
TIGERBOT:set(TIGER_ID.."all:addreply1:photo:gp"..test..bot_id, photo_in_group)  
TIGERBOT:set(TIGER_ID.."all:addreply1:photo:caption:gp"..test..bot_id,(msg.content_.caption_ or ''))  
end
TIGERBOT:del(TIGER_ID.."addreply2:"..msg.sender_user_id_..bot_id)    
return false    
end    
end
if text and msg.reply_to_message_id_ ~= 0 and not TIGERBOT:get(TIGER_ID..'lock:rep:all:rd'..bot_id) then   
local anemi = TIGERBOT:get(TIGER_ID.."add:repallt:gif:all:rd"..text..bot_id)    
local veico = TIGERBOT:get(TIGER_ID.."add:rep:tvico:all:rd"..text..bot_id)    
local stekr = TIGERBOT:get(TIGER_ID.."add:rep:tstekr:all:rd"..text..bot_id)      
local taha = TIGERBOT:get(TIGER_ID.."add:rep:text:all:rd"..text..bot_id)    
local video_caption = TIGERBOT:get(TIGER_ID.."all:addreply1:video:caption:gp"..text..bot_id)
local photo_caption = (TIGERBOT:get(TIGER_ID.."all:addreply1:photo:caption:gp"..text..bot_id) or '' )
local photo = TIGERBOT:get(TIGER_ID.."all:addreply1:photo:gp"..text..bot_id)
local video = TIGERBOT:get(TIGER_ID.."all:addreply1:video:gp"..text..bot_id)
local document = TIGERBOT:get(TIGER_ID.."all:addreply1:document:gp"..text..bot_id)
local audio = TIGERBOT:get(TIGER_ID.."all:addreply1:audio:gp"..text..bot_id)
if taha then     
TIGER_sendMsg(msg.chat_id_, msg.reply_to_message_id_, 1, ''..check_markdown(taha)..'', 1, 'md')      
return false    
end     
...
if TIGERBOT:get(TIGER_ID..'lock:rep:all'..bot_id) then
taha = '*☑┇ تم تفعيل ردود المطور *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:del(TIGER_ID..'lock:rep:all'..bot_id)   
else
...
if not TIGERBOT:get(TIGER_ID..'lock:rep:all'..bot_id) then
taha = '*☑┇ تم تعطيل ردود المطور *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:set(TIGER_ID..'lock:rep:all'..bot_id,true)   
else
...
local taha = TIGERBOT:scard(TIGER_ID.."rep:media:all"..bot_id)    
for k,v in pairs(list) do    
TIGERBOT:del(TIGER_ID.."add:repallt"..msg.sender_user_id_..bot_id)    
TIGERBOT:del(TIGER_ID.."add:repallt:gif:all"..v..bot_id)    
TIGERBOT:del(TIGER_ID.."add:rep:tvico:all"..v..bot_id)    
TIGERBOT:del(TIGER_ID.."add:rep:tstekr:all"..v..bot_id)    
TIGERBOT:del(TIGER_ID.."add:rep:text:all"..v..bot_id)    
TIGERBOT:del(TIGER_ID.."mall:addreply1:photo:gp"..v..bot_id)
TIGERBOT:del(TIGER_ID.."mall:addreply1:video:gp"..v..bot_id)
TIGERBOT:del(TIGER_ID.."mall:addreply1:document:gp"..v..bot_id)
TIGERBOT:del(TIGER_ID.."mall:addreply1:audio:gp"..v..bot_id)
TIGERBOT:srem(TIGER_ID.."rep:media:all"..bot_id,v)    
TIGERBOT:del("add:repallt"..msg.sender_user_id_..bot_id)    
TIGERBOT:del("add:repallt:gif:all"..v..bot_id)    
TIGERBOT:del("add:rep:tvico:all"..v..bot_id)    
TIGERBOT:del("add:rep:tstekr:all"..v..bot_id)    
TIGERBOT:del("add:rep:text:all"..v..bot_id)    
TIGERBOT:del("mall:addreply1:photo:gp"..v..bot_id)
TIGERBOT:del("mall:addreply1:video:gp"..v..bot_id)
TIGERBOT:del("mall:addreply1:document:gp"..v..bot_id)
...
local list = TIGERBOT:smembers(TIGER_ID.."rep:media:all"..bot_id)    
t = "*❕┇ قائمة ردود المطور ✔🙋🏻‍♂️※\nٴ━━━━━━━━━━━*\n"    
for k,v in pairs(list) do    
if TIGERBOT:get(TIGER_ID.."add:repallt:gif:all"..v..bot_id) then
taha = 'متحركه 🎆'
elseif TIGERBOT:get(TIGER_ID.."add:rep:tvico:all"..v..bot_id) then
taha = 'بصمه 🎙'
elseif TIGERBOT:get(TIGER_ID.."add:rep:tstekr:all"..v..bot_id) then
taha = 'ملصق 🃏'
elseif TIGERBOT:get(TIGER_ID.."add:rep:text:all"..v..bot_id) then
taha = 'رساله 💭'
elseif TIGERBOT:get(TIGER_ID.."mall:addreply1:photo:gp"..v..bot_id)  then
taha = 'صوره 🌇'
elseif TIGERBOT:get(TIGER_ID.."mall:addreply1:video:gp"..v..bot_id)  then
taha = 'فيديو 📹'
elseif TIGERBOT:get(TIGER_ID.."mall:addreply1:document:gp"..v..bot_id)  then
taha = 'ملف 📁'
elseif TIGERBOT:get(TIGER_ID.."mall:addreply1:audio:gp"..v..bot_id)  then
taha = 'اغنيه 🎵'
end
t = t..'*'..k..'• *❨`'..v..'`❩ *» {'..taha..'}*\n'    
end    
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*💬┇ حسنآ ارسل لي الكلمه الان *\n✔🙋🏻‍♂️※', 1, 'md')  
TIGERBOT:set(TIGER_ID.."add:repallt"..msg.sender_user_id_..bot_id,'yes')    
return false    end      
if text then    
local tt = TIGERBOT:get(TIGER_ID.."add:repallt"..msg.sender_user_id_..bot_id)    
if tt == 'yes' then     
if text and text:match("^الغاء$") then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*☑┇ تم الغاء الامر *\n✓※", 1, "md") 
TIGERBOT:del(TIGER_ID.."add:repallt"..msg.sender_user_id_..bot_id)    
return false  end 
if TIGERBOT:sismember(TIGER_ID.."rep:media:all"..bot_id,text) then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*💢┇ لقد تم اضافة رد بهاذه الكلمه \n💬┇ ارسل كلمه اخرى او ارسل الغاء*\n✔🙋🏻‍♂️※\n", 1, 'md')  
else
media = '{ متحركه ‹› ملصق ‹› صوره ‹› اغنيه ‹› بصمه ‹› ملف ‹› فيديو }'
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*💬┇ حسنآ ارسل الرد الان\n💢┇ يمكنك ارسال الرد » "..media.."*\n✔🙋🏻‍♂️※", 1, 'md')  
TIGERBOT:set(TIGER_ID.."add:repallt"..msg.sender_user_id_..bot_id,'yes1')    
TIGERBOT:set(TIGER_ID.."addreply2:"..msg.sender_user_id_..bot_id, text)    
TIGERBOT:sadd(TIGER_ID.."rep:media:all"..bot_id,text)    
end
return false    end    
end
if text and text == 'حذف رد عام' and  is_devtaha(msg) then     
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*💬┇ حسنآ ارسل لي الكلمه الان *\n✔🙋🏻‍♂️※', 1, 'md')  
TIGERBOT:set(TIGER_ID.."add:repallt"..msg.sender_user_id_..bot_id,'yes11')    
return false    end    
if text then 
local test = TIGERBOT:get(TIGER_ID.."add:repallt"..msg.sender_user_id_..bot_id)    
if test and test == 'yes11' then   
if text and text:match("^الغاء$") then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*☑┇ تم الغاء الامر *\n✓※", 1, "md") 
TIGERBOT:del(TIGER_ID.."add:repallt"..msg.sender_user_id_..bot_id)    
return false  end   
if not TIGERBOT:sismember(TIGER_ID..'rep:media:all'..bot_id,text) then
TIGERBOT:del(TIGER_ID.."add:repallt"..msg.sender_user_id_..bot_id)    
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*📊┇ الكلمه » {* ["..text.."] *} ※\n🚫┇ لا يوجد رد بهاذه الكلمه*\n✔🙋🏻‍♂️※\n", 1, 'md')  
TIGERBOT:del(TIGER_ID.."add:repallt:gif:all"..text..bot_id)    TIGERBOT:del(TIGER_ID.."add:rep:tvico:all"..text..bot_id)    TIGERBOT:del(TIGER_ID.."add:rep:tstekr:all"..text..bot_id)    TIGERBOT:del(TIGER_ID.."add:rep:text:all"..text..bot_id) TIGERBOT:srem("rep:media:all"..bot_id,text) 
else
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*📊┇ الكلمه » {* ["..text.."] *} ※\n💢┇ تم حذفها من قائمة الردود *\n✔🙋🏻‍♂️※\n", 1, 'md')  
TIGERBOT:del(TIGER_ID.."add:repallt"..msg.sender_user_id_..bot_id)    
TIGERBOT:del(TIGER_ID.."add:repallt:gif:all"..text..bot_id)    
TIGERBOT:del(TIGER_ID.."add:rep:tvico:all"..text..bot_id)    
TIGERBOT:del(TIGER_ID.."add:rep:tstekr:all"..text..bot_id)    
TIGERBOT:del(TIGER_ID.."add:rep:text:all"..text..bot_id)    
TIGERBOT:del(TIGER_ID.."mall:addreply1:photo:gp"..text..bot_id)
TIGERBOT:del(TIGER_ID.."mall:addreply1:video:gp"..text..bot_id)
TIGERBOT:del(TIGER_ID.."mall:addreply1:document:gp"..text..bot_id)
TIGERBOT:del(TIGER_ID.."mall:addreply1:audio:gp"..text..bot_id)
TIGERBOT:srem(TIGER_ID.."rep:media:all"..bot_id,text)    
end
return false    end    
end    
if text or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.animation_ or msg.content_.audio_ or msg.content_.document_ or msg.content_.photo_ or msg.content_.video_ then  
...
local test = TIGERBOT:get(TIGER_ID.."addreply2:"..msg.sender_user_id_..bot_id)    
if msg.content_.sticker_ then     
TIGERBOT:set(TIGER_ID.."add:rep:tstekr:all"..test..bot_id, msg.content_.sticker_.sticker_.persistent_id_)    
end     
if msg.content_.voice_ then    
TIGERBOT:set(TIGER_ID.."add:rep:tvico:all"..test..bot_id, msg.content_.voice_.voice_.persistent_id_)    
end     
if msg.content_.animation_ then     
TIGERBOT:set(TIGER_ID.."add:repallt:gif:all"..test..bot_id, msg.content_.animation_.animation_.persistent_id_)    
end    
if text then     
TIGERBOT:set(TIGER_ID.."add:rep:text:all"..test..bot_id, text)    
end    
if msg.content_.audio_ then
TIGERBOT:set(TIGER_ID.."mall:addreply1:audio:gp"..test..bot_id, msg.content_.audio_.audio_.persistent_id_)  
end
if msg.content_.document_ then
TIGERBOT:set(TIGER_ID.."mall:addreply1:document:gp"..test..bot_id, msg.content_.document_.document_.persistent_id_)  
end
if msg.content_.video_ then
TIGERBOT:set(TIGER_ID.."mall:addreply1:video:gp"..test..bot_id, msg.content_.video_.video_.persistent_id_)  
TIGERBOT:set(TIGER_ID.."mall:addreply1:video:caption:gp"..test..bot_id,(msg.content_.caption_ or ''))  
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo_in_group = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
TIGERBOT:set(TIGER_ID.."mall:addreply1:photo:gp"..test..bot_id, photo_in_group)  
TIGERBOT:set(TIGER_ID.."mall:addreply1:photo:caption:gp"..test..bot_id,(msg.content_.caption_ or ''))  
end
TIGERBOT:del(TIGER_ID.."addreply2:"..msg.sender_user_id_..bot_id)    
return false    end    
end
if text and not TIGERBOT:get(TIGER_ID..'lock:rep:all'..bot_id) then    
local anemi = TIGERBOT:get(TIGER_ID.."add:repallt:gif:all"..text..bot_id)    
local veico = TIGERBOT:get(TIGER_ID.."add:rep:tvico:all"..text..bot_id)    
local stekr = TIGERBOT:get(TIGER_ID.."add:rep:tstekr:all"..text..bot_id)      
local taha = TIGERBOT:get(TIGER_ID.."add:rep:text:all"..text..bot_id)    
local video_caption = TIGERBOT:get(TIGER_ID.."mall:addreply1:video:caption:gp"..text..bot_id)
local photo_caption = (TIGERBOT:get(TIGER_ID.."mall:addreply1:photo:caption:gp"..text..bot_id) or '' )
local photo = TIGERBOT:get(TIGER_ID.."mall:addreply1:photo:gp"..text..bot_id)
local video = TIGERBOT:get(TIGER_ID.."mall:addreply1:video:gp"..text..bot_id)
local document = TIGERBOT:get(TIGER_ID.."mall:addreply1:document:gp"..text..bot_id)
local audio = TIGERBOT:get(TIGER_ID.."mall:addreply1:audio:gp"..text..bot_id)
if taha then     
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, ''..check_markdown(taha)..'', 1, 'md')      
return false    end     
if veico then     
...
local group = TIGERBOT:smembers(TIGER_ID..'bot:gpsby:id')   
local w = 0
local q = 0
for i = 1, #group do
...
TIGERBOT:srem(TIGER_ID.."bot:gpsby:id",group[i])  
rem_group(group[i])   
...
if #group == i then 
if (w + q) == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*🚫┇ لا يوجد مجموعات وهميه في البوت* \n✔🙋🏻‍♂️※', 1, 'md')   
else
local taha = (w + q)
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*🎴┇ عدد المجموعات الان ↫ ❪ '..#group..' ❫*'..TIGER..''..taha..'\n*🔖┇ الان عدد المجموعات الحقيقي ↫ ❪ '..sendok..' ❫ مجموعات*\n✔🙋🏻‍♂️※', 1, 'md')   
end
end
end,nil)
end
...
local pv = TIGERBOT:smembers(TIGER_ID..'usersbot')   
local sendok = 0
for i = 1, #pv do
...
TIGERBOT:srem(TIGER_ID.."usersbot",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*🚫┇ لا يوجد مشتركين وهميين في البوت* \n✔🙋🏻‍♂️※', 1, 'md')   
else
local ok = #pv - sendok
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*💢┇ عدد المشتركين الان ↫ ❪ '..#pv..' ❫*\n*☑┇ تم ازالة ↫ ❪ '..sendok..' ❫ من المشتركين*\n*🔖┇ الان عدد المشتركين الحقيقي ↫ ❪ '..ok..' ❫ مشترك*\n✔🙋🏻‍♂️※', 1, 'md')   
end
end
end,nil)
end,nil)
...
if (c - x) == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*🚫┇ لا توجد بوتات هنا *\n ", 1, 'md')
else
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, text..a..f, 1, 'md')
end
end,nil)
end
end
...
TIGERBOT:hset("flooding:settings:"..msg.chat_id_ ,"flood",'kick')  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '☑*┇* تم قفل التكرار بالطرد \n*🔖┇ خـاصيــه ، الطرد ✔🙋🏻‍♂️※*\n✔🙋🏻‍♂️※',1, 'md')
elseif text == 'قفل التكرار' and is_mod(msg) then 
TIGERBOT:hset("flooding:settings:"..msg.chat_id_ ,"flood",'del')  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '☑*┇* تم قفل التكرار \n*🔖┇ خـاصيــه ، الحذف ✔🙋🏻‍♂️※*\n✔🙋🏻‍♂️※',1, 'md')
elseif text == 'قفل التكرار بالتقييد' and is_mod(msg) then 
TIGERBOT:hset("flooding:settings:"..msg.chat_id_ ,"flood",'keed')  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '☑*┇* تم قفل التكرار بالتقييد \n*🔖┇ خـاصيــه ، التقييد ✔🙋🏻‍♂️※*\n✔🙋🏻‍♂️※',1, 'md')
elseif text == 'قفل التكرار بالكتم' and is_mod(msg) then 
TIGERBOT:hset("flooding:settings:"..msg.chat_id_ ,"flood",'mute')  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '☑*┇* تم قفل التكرار بالكتم \n*🔖┇ خـاصيــه ، الكتم ✔🙋🏻‍♂️※*\n✔🙋🏻‍♂️※',1, 'md')
elseif text == 'فتح التكرار' and is_mod(msg) then 
TIGERBOT:hdel("flooding:settings:"..msg.chat_id_ ,"flood")  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '☑*┇* تم فتح التكرار \n✔🙋🏻‍♂️※',1, 'md')
end 
if TIGERBOT:get(TIGER_ID.."bc:gp" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
TIGERBOT:del(TIGER_ID..'id:gp'..msg.chat_id_)  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*☑┇ تم الغاء الامر *\n✓※", 1, "md") 
TIGERBOT:del(TIGER_ID.."bc:gp" .. msg.chat_id_ .. "" .. msg.sender_user_id_)  
return false  end 
TIGERBOT:del(TIGER_ID.."bc:gp" .. msg.chat_id_ .. "" .. msg.sender_user_id_)  
local numadded = string.match(text, "(.*)") 
local iduserr = TIGERBOT:get(TIGER_ID..'id:gp'..msg.chat_id_)  
TIGER_sendMsg((iduserr), 0, 1, numadded, 1, "html")   
TIGER_sendMsg(msg.chat_id_, msg.id_,  1, "☑*┇* تم ارسال رسالتك الى  *{ "..iduserr..' }* ', 1, 'md')  
end
if text and text:match("^اذاعه (-%d+)$") and is_devtaha(msg) then  
taha = text:match("^اذاعه (-%d+)$")
TIGERBOT:set(TIGER_ID..'id:gp'..msg.chat_id_,taha)  
TIGERBOT:setex(TIGER_ID.."bc:gp" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)  
local t = '*💬┇ ارسل لي النص الذي تريده*'  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,t, 1, 'md') 
end
if text then 
if is_mod(msg) then
if text == 'ارسال نسخه' and is_devtaha(msg) then  sendDocument(SUDO, 0, 0, 1, nil, './TIGER.lua', '🚫┇اسم الملف ( TIGER.lua )\n♻┇عدد المشتركين ( '..(TIGERBOT:scard(TIGER_ID.."usersbot") or 0)..' )\n📮┇عدد المجموعات ( '..(TIGERBOT:scard(TIGER_ID.."botgps") or 0)..' )',dl_cb, nil)  end
if text == 'اذاعه خاص' and tonumber(msg.reply_to_message_id_) > 0 and is_devtaha(msg) then 
function cb(a,b,c) 
if b.content_.text_ then
local list = TIGERBOT:smembers(TIGER_ID..'usersbot') 
for k,v in pairs(list) do 
TIGER_sendMsg(v, 0, 1, '[ '..b.content_.text_..' ]', 1, 'md')  
end
...
local pv = TIGERBOT:scard(TIGER_ID.."usersbot")      
...
local grp = TIGERBOT:scard(TIGER_ID.."bot:gpsby:id")       
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '♻ • جاري تحديث السورس • ♻', 1, 'md') 
os.execute('rm -rf TIGER.lua') 
os.execute("rm -fr plugins_/help_rep.lua")
os.execute("rm -fr plugins_/ZHRFA.lua")
os.execute("rm -fr plugins_/games.lua")
...
local grall = TIGERBOT:scard(TIGER_ID.."botgps") or 0  
local gradd = TIGERBOT:scard(TIGER_ID..'bot:gpsby:id') or 0  
local uspv = TIGERBOT:scard(TIGER_ID.."usersbot") or 0  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*\n🀄┇ عدد المجموعات المفعله ↫ ❪'..gradd..'❫\n🔖┇ عدد المشتركين ↫ ❪'..uspv..'❫*\n✓※', 1, 'md') 
end

if text ==  ""..NAMEBOT..' غادر' and is_owner(msg) then 
TIGERBOT:srem(TIGER_ID.."addgrop", msg.chat_id_) 
TIGERBOT:del(TIGER_ID.."add:bot:group"..msg.chat_id_, true) 
rem_group(msg.chat_id_)  
rem_lockal(msg.chat_id_) 
TIGERBOT:del(TIGER_ID.."test:group"..msg.chat_id_)   
TIGERBOT:del(TIGER_ID..'moder'..msg.chat_id_) 
TIGERBOT:del(TIGER_ID..'modergroup'..msg.chat_id_) 
TIGERBOT:del(TIGER_ID..'mods:'..msg.chat_id_) 
TIGERBOT:del(TIGER_ID.."add:bot:group"..msg.chat_id_, true) 
TIGERBOT:srem(TIGER_ID..'bot:gpsby:id', msg.chat_id_)  
TIGERBOT:srem(TIGER_ID.."botgps", msg.chat_id_)   
changeChatMemberStatus(msg.chat_id_, bot_id, "Left") 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '☑*┇*  تم مغادرة المجموعه ', 1, 'md') 
return false  
end
if text == 'مسح المقيدين' and is_mod(msg) then     
local list = TIGERBOT:smembers(TIGER_ID..'tedmembars'..msg.chat_id_) 
local pv = 0
for k,v in pairs(list) do   
TIGERBOT:del(TIGER_ID..'tedmembars'..msg.chat_id_) 
TIGERBOT:srem(TIGER_ID.."keed", msg.chat_id_)  
...
if pv == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'🚫*┇* لا يوجد مقيدين هنا\n', 1, 'md')   
else
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'☑*┇ تم مسح {'..pv..'} من المقيدين*\n', 1, 'md')   
end
end
...
return false  end
if kk.id_ then  
local msgss = tonumber(TIGERBOT:get(TIGER_ID..'user:messages:'..msg.chat_id_..':'..kk.id_) or 0)  
...
t = 'مطور اساسي'
elseif TIGERBOT:sismember(TIGER_ID..'sudo:bot',kk.id_) then
t = 'المطور'
elseif TIGERBOT:sismember(TIGER_ID..'moder'..msg.chat_id_,kk.id_) then
t = 'المنشئ'
elseif TIGERBOT:sismember(TIGER_ID..'modergroup'..msg.chat_id_,kk.id_) then
t = 'المدير'
elseif TIGERBOT:sismember(TIGER_ID..'mods:'..msg.chat_id_,kk.id_) then
t = 'الادمن'
elseif TIGERBOT:sismember(TIGER_ID..'vip:groups',kk.id_) then
t = 'مميز عام'
elseif TIGERBOT:sismember(TIGER_ID..'vip:group'..msg.chat_id_,kk.id_) then
t = 'عضو مميز'
else
t = 'مجرد عضو'
end
...
if TIGERBOT:sismember(TIGER_ID..'TIGER:gbaned',kk.id_) then
kkeed = 'محظور عام'
elseif TIGERBOT:sismember(TIGER_ID..'TIGER:baned'..msg.chat_id_,kk.id_) then
kkeed = 'محظور'
elseif TIGERBOT:sismember(TIGER_ID..'mutes'..msg.chat_id_,kk.id_) then
kkeed = 'مكتوم'
elseif TIGERBOT:sismember(TIGER_ID..'tedmembars'..msg.chat_id_,kk.id_) then
kkeed = 'مقيد'
else
kkeed = ' لا يوجد'
end
...
'` ❫\n⭐*┇ رتبـة الكروب » ❪ '..rtpa..
' ❫\n🎖┇ رتبـة البوت » ❪ '..t..
' ❫\n📨┇ رسـائله » ❪ '..(TIGERBOT:get(TIGER_ID..'user:messages:'..msg.chat_id_..':'..kk.id_) or 0)..
...
local msgss = tonumber(TIGERBOT:get(TIGER_ID..'user:messages:'..msg.chat_id_..':'..iduser) or 0)  
...
t = 'مطور اساسي'
elseif TIGERBOT:sismember(TIGER_ID..'sudo:bot',iduser) then
t = 'المطور'
elseif TIGERBOT:sismember(TIGER_ID..'moder'..msg.chat_id_,iduser) then
t = 'المنشئ'
elseif TIGERBOT:sismember(TIGER_ID..'modergroup'..msg.chat_id_,iduser) then
t = 'المدير'
elseif TIGERBOT:sismember(TIGER_ID..'mods:'..msg.chat_id_,iduser) then
t = 'الادمن'
elseif TIGERBOT:sismember(TIGER_ID..'vip:groups',iduser) then
t = 'مميز عام'
elseif TIGERBOT:sismember(TIGER_ID..'vip:group'..msg.chat_id_,iduser) then
t = 'عضو مميز'
else
t = 'مجرد عضو'
end
...
if TIGERBOT:sismember(TIGER_ID..'TIGER:gbaned',iduser) then
kkeed = 'محظور عام'
elseif TIGERBOT:sismember(TIGER_ID..'TIGER:baned'..msg.chat_id_,iduser) then
kkeed = 'محظور'
elseif TIGERBOT:sismember(TIGER_ID..'mutes'..msg.chat_id_,iduser) then
kkeed = 'مكتوم'
elseif TIGERBOT:sismember(TIGER_ID..'tedmembars'..msg.chat_id_,iduser) then
kkeed = 'مقيد'
else
kkeed = ' لا يوجد'
end
...
' }❫\n⭐┇ رتبـة الكروب » ❪ '..rtpa..
' ❫\n🎖┇ رتبـة البوت » ❪ '..t..
' ❫\n📨┇ رسـائله » ❪ '..(TIGERBOT:get(TIGER_ID..'user:messages:'..msg.chat_id_..':'..iduser) or 0)..
' ❫\n🗳┇ تفــاعله » ❪ '..formsgg(msgss)..' ❫'..photouser1..
'\n🚫┇ نوع القيود » ❪ '..kkeed..
' ❫\n🔹┇ التواجد » ❪ '..tt..
' ❫\n⚡┇ نوع حسابه » ❪ '..acca..' ❫'
...
local msgss = tonumber(TIGERBOT:get(TIGER_ID..'user:messages:'..msg.chat_id_..':'..result.sender_user_id_) or 0)  
...
t = 'مطور اساسي'
elseif TIGERBOT:sismember(TIGER_ID..'sudo:bot',result.sender_user_id_) then
t = 'المطور'
elseif TIGERBOT:sismember(TIGER_ID..'moder'..msg.chat_id_,result.sender_user_id_) then
t = 'المنشئ'
elseif TIGERBOT:sismember(TIGER_ID..'modergroup'..msg.chat_id_,result.sender_user_id_) then
t = 'المدير'
elseif TIGERBOT:sismember(TIGER_ID..'mods:'..msg.chat_id_,result.sender_user_id_) then
t = 'الادمن'
elseif TIGERBOT:sismember(TIGER_ID..'vip:groups',result.sender_user_id_) then
t = 'مميز عام'
elseif TIGERBOT:sismember(TIGER_ID..'vip:group'..msg.chat_id_,result.sender_user_id_) then
t = 'عضو مميز'
else
t = 'مجرد عضو'
end
...
kkeed = 'محظور عام'
elseif TIGERBOT:sismember(TIGER_ID..'TIGER:baned'..msg.chat_id_,result.sender_user_id_) then
kkeed = 'محظور'
elseif TIGERBOT:sismember(TIGER_ID..'mutes'..msg.chat_id_,result.sender_user_id_) then
kkeed = 'مكتوم'
elseif TIGERBOT:sismember(TIGER_ID..'tedmembars'..msg.chat_id_,result.sender_user_id_) then
kkeed = 'مقيد'
else
kkeed = 'لا يوجد'
end
...
' }❫\n⭐┇ رتبـة الكروب » ❪ '..rtpa..
' ❫\n🎖┇ رتبـة البوت » ❪ '..t..
' ❫\n📨┇ رسـائله » ❪ '..(TIGERBOT:get(TIGER_ID..'user:messages:'..msg.chat_id_..':'..result.sender_user_id_) or 0)..
' ❫\n🗳┇ تفــاعله » ❪ '..formsgg(msgss)..' ❫'..photouser1..
'\n🚫┇ نوع القيود » ❪ '..kkeed..
' ❫\n🔹┇ التواجد » ❪ '..tt..
' ❫\n⚡┇ نوع حسابه » ❪ '..acca..' ❫'
...
local msgss = tonumber(TIGERBOT:get(TIGER_ID..'user:messages:'..msg.chat_id_..':'..tes.content_.entities_[0].user_id_) or 0)  
...
t = 'مطور اساسي'
elseif TIGERBOT:sismember(TIGER_ID..'sudo:bot',tes.content_.entities_[0].user_id_) then
t = 'المطور'
elseif TIGERBOT:sismember(TIGER_ID..'moder'..msg.chat_id_,tes.content_.entities_[0].user_id_) then
t = 'المنشئ'
elseif TIGERBOT:sismember(TIGER_ID..'modergroup'..msg.chat_id_,tes.content_.entities_[0].user_id_) then
t = 'المدير'
elseif TIGERBOT:sismember(TIGER_ID..'mods:'..msg.chat_id_,tes.content_.entities_[0].user_id_) then
t = 'الادمن'
elseif TIGERBOT:sismember(TIGER_ID..'vip:groups',tes.content_.entities_[0].user_id_) then
t = 'مميز عام'
elseif TIGERBOT:sismember(TIGER_ID..'vip:group'..msg.chat_id_,tes.content_.entities_[0].user_id_) then
t = 'عضو مميز'
else
t = 'مجرد عضو'
end
...
if TIGERBOT:sismember(TIGER_ID..'TIGER:gbaned',tes.content_.entities_[0].user_id_) then
kkeed = 'محظور عام'
elseif TIGERBOT:sismember(TIGER_ID..'TIGER:baned'..msg.chat_id_,tes.content_.entities_[0].user_id_) then
kkeed = 'محظور'
elseif TIGERBOT:sismember(TIGER_ID..'mutes'..msg.chat_id_,tes.content_.entities_[0].user_id_) then
kkeed = 'مكتوم'
elseif TIGERBOT:sismember(TIGER_ID..'tedmembars'..msg.chat_id_,tes.content_.entities_[0].user_id_) then
kkeed = 'مقيد'
else
kkeed = ' لا يوجد'
end
...
' }❫\n⭐┇ رتبـة الكروب » ❪ '..rtpa..
' ❫\n🎖┇ رتبـة البوت » ❪ '..t..
' ❫\n📨┇ رسـائله » ❪ '..(TIGERBOT:get(TIGER_ID..'user:messages:'..msg.chat_id_..':'..tes.content_.entities_[0].user_id_) or 0)..
' ❫\n🗳┇ تفــاعله » ❪ '..formsgg(msgss)..' ❫'..photouser1..
'\n🚫┇ نوع القيود » ❪ '..kkeed..
' ❫\n🔹┇ التواجد » ❪ '..tt..
' ❫\n⚡┇ نوع حسابه » ❪ '..acca..' ❫'
...
TIGERBOT:setex(TIGER_ID.."get:link:gp" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)  
local t = '*💬┇ حسنآ ارسل لي ايدي المجموعه*\n✔🙋🏻‍♂️※'  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,t, 1, 'md') 
end
if text and text:match("^معلومات$") and is_devtaha(msg) then  
TIGERBOT:setex(TIGER_ID.."get:info:gp" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)  
local t = '*💬┇ حسنآ ارسل لي ايدي المجموعه*\n✔🙋🏻‍♂️※'  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,t, 1, 'md') 
end
if text == 'الكروبات' and is_sudo(msg) then 
local t = TIGERBOT:scard(TIGER_ID.."botgps")
local y = TIGERBOT:scard(TIGER_ID.."bot:gpsby:id") 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🎴┇ العدد الكلي للكروبات هو » ❪'..(y)..'❫* \n', 1, 'md') 
end
if text == 'المشتركين' and is_sudo(msg) then     
local addgrop = TIGERBOT:scard(TIGER_ID.."usersbot") or 0    
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🔖️┇ عدد المشتركين في البوت » ❪ '..addgrop..' ❫*\n✔🙋🏻‍♂️※', 1, 'md')    
end      
...
TIGERBOT:sadd(TIGER_ID..'mods:'..msg.chat_id_,admins[i].user_id_) 
num2 = num2 + 1
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_
},function(arg,b) 
if b.username_ == true then
TIGERBOT:set(TIGER_ID.."user:Name"..b.id_,"@"..b.username_)
end
if b.first_name_ == false then
TIGERBOT:srem(TIGER_ID..'mods:'..msg.chat_id_,admins[i].user_id_) 
end
end,nil)   
else
TIGERBOT:srem(TIGER_ID..'mods:'..msg.chat_id_,admins[i].user_id_) 
...
if b.first_name_ == false then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*💢┇ حساب المنشئ محذوف *\n', 1, 'md')
TIGERBOT:srem(TIGER_ID..'moder'..msg.chat_id_,owner_id) 
return false  end
...
if data.members_[i].status_.ID == "ChatMemberStatusCreator" then
owner_id = admins[i].user_id_
TIGERBOT:sadd(TIGER_ID..'moder'..msg.chat_id_,owner_id) 
end
end
tdcli_function ({ID = "GetUser",user_id_ = owner_id
},function(arg,b) 
if b.first_name_ == false then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*🚫┇ المنشئ حاذف لا استطيع رفعه منشئ *\n', 1, 'md')
TIGERBOT:srem(TIGER_ID..'moder'..msg.chat_id_,owner_id) 
return false  end
if b.username_ == false then 
local text = '☑┇ تم ترقية منشئ المجموعه \n•※ '..CatchName(b.first_name_,50)..' ※• \n✓'
TIGERmonshn(msg.chat_id_, owner_id, msg.id_, text, 31, utf8.len(b.first_name_)) 
else
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*☑┇ تم ترقية منشئ المجموعه \n •※* ['..CatchName(b.first_name_,20)..'](t.me/'..b.username_..') *※• *\n✓※', 1, 'md') 
TIGERBOT:set(TIGER_ID.."user:Name"..b.id_,"@"..b.username_)
end
end,nil)   
end,nil)   
...
TIGER_sendMsg(msg.chat_id_,msg.id_, 1, "*❕┇* اني مو ادمن هنا  \n", 1, 'md')  
return false  end 
if data.message_ == "CHAT_ADMIN_REQUIRED" then
TIGER_sendMsg(msg.chat_id_,msg.id_, 1, "*❕┇* ماعندي صلاحيه اغير اسم المجموعه \n", 1, 'md')  
else
TIGER_sendMsg(msg.chat_id_,msg.id_, 1, "*☑┇* تم وضع اسم للمجموعه \n", 1, 'md')  
TIGERBOT:set(TIGER_ID..'group:name'..msg.chat_id_,name)
end
end,nil) 
end
if text=="ضع رابط" and msg.reply_to_message_id_ == 0  and is_mod(msg) then  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*💬┇* حسناا ارسل لي رابط المجموعه ", 1, 'md')       
TIGERBOT:set(TIGER_ID.."link:group"..msg.chat_id_, 'setlinkwai') 
end
if text == "الرابط" then 
local link = TIGERBOT:get(TIGER_ID.."link:group"..msg.chat_id_)            
if link then                              
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*ٴ📮┇ »* رابط مجموعة ↓\n*ٴ🎴┇ » '..TIGERBOT:get(TIGER_ID..'group:name'..msg.chat_id_)..' *\n*ٴ📛┇* » ['..link..']\n✔🙋🏻‍♂️※', 1, 'md')                          
else                
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🚫┇* لا يوجد رابط المجموعه\n*🔖┇ ارسل » ❪ ضع رابط ❫ لوضع رابط المجموعه*\n✔🙋🏻‍♂️※', 1, 'md')              
end            
end
if text and text:match("^مسح الرابط$") and is_mod(msg) then              
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*☑┇* تم مسح رابط المجموعه \n✓※", 1 , "md")           
TIGERBOT:del(TIGER_ID.."link:group" .. msg.chat_id_)       
end
if text=="اذاعه بالتوجيه" and msg.reply_to_message_id_ == 0  and is_devtaha(msg) then 
TIGERBOT:setex(TIGER_ID.."bc:in:gropsfwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*💬┇* ارسل لي التوجيه ليتم اذاعته للمجموعات\n✓※", 1, "md") 
end
if text=="اذاعه خاص بالتوجيه" and msg.reply_to_message_id_ == 0  and is_devtaha(msg) then 
TIGERBOT:setex(TIGER_ID.."bc:in:pvfwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*💬┇* ارسل لي التوجيه ليتم اذاعته للخاص\n✓※", 1, "md") 
end
if text=="اذاعه عام بالتوجيه" and msg.reply_to_message_id_ == 0  and is_devtaha(msg) then 
TIGERBOT:setex(TIGER_ID.."bc:in:allfwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*💬┇* ارسل لي التوجيه ليتم اذاعته للكل\n✓※", 1, "md") 
end
if text=="اذاعه" and msg.reply_to_message_id_ == 0  and is_devtaha(msg) then 
TIGERBOT:setex(TIGER_ID.."bc:in:grops" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*💬┇* ارسل لي سواء كان »❪ رساله , صوره , متحركه , ملصق  ❫ للاذاعه الى المجموعات\n✓※", 1, "md") 
end
if text=="اذاعه للكل" and msg.reply_to_message_id_ == 0  and is_devtaha(msg) then 
TIGERBOT:setex(TIGER_ID.."bc:in:all" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*💬┇* ارسل لي سواء كان »❪ رساله , صوره , متحركه , ملصق  ❫ للاذاعه الى الكل\n✓※", 1, "md") 
end
if text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 and is_devtaha(msg) then 
TIGERBOT:setex(TIGER_ID.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*💬┇* ارسل لي سواء كان »❪ رساله , صوره , متحركه , ملصق  ❫ للاذاعه الى الخاص\n✓※", 1, "md") 
end 
if text and text:match("^ضع قوانين$") and is_mod(msg) then 
TIGERBOT:setex(TIGER_ID.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "💬*┇* ارسل لي النص الان \n✔🙋🏻‍♂️※", 1, "md")  
end
if text and text:match("^مسح القوانين$")  and is_mod(msg) then  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "☑*┇* تم مسح القوانين \n✓※", 1, "md")  
TIGERBOT:del(TIGER_ID.."rules:group" .. msg.chat_id_) 
end
if text and text:match("^القوانين$") then 
local rules = TIGERBOT:get(TIGER_ID.."rules:group" .. msg.chat_id_)   
if rules then     
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, rules, 1, nil)   
else      
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*🚫┇* لا يوجد قوانين هنا \n✔🙋🏻‍♂️※",  1, "md")   
end    
end
...
TIGER_sendMsg(msg.chat_id_, result.id_, 1,''..taha[math.random(#taha)]..'', 1, 'html') 
else  
end 
end 
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, 'دنجب لك 😂 تريدني احجي عله  روحي\n', 1, 'md')  
return false  end  
if tonumber(result.sender_user_id_) == tonumber(SUDO) then  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, 'شو انته كاعد تمضرط غير هاذا المطور مالتي 😌\n ', 1, 'md')  
return false  end 
local taha = { 
...
TIGER_sendMsg(msg.chat_id_, result.id_, 1,''..taha[math.random(#taha)]..'', 1, 'md') 
end 
...
TIGERBOT:srem(TIGER_ID..'bot:gps',gp) 
TIGERBOT:srem(TIGER_ID..'bot:gpsby:id',gp) 
TIGERBOT:srem(TIGER_ID.."botgps",gp) 
TIGERBOT:del(TIGER_ID..'mod:'..gp) 
TIGERBOT:del(TIGER_ID..'moder'..gp) 
TIGERBOT:del(TIGER_ID..'banned:'..gp) 
TIGERBOT:del(TIGER_ID.."add:bot:group"..gp) 
TIGERBOT:del(TIGER_ID.."setmoder:"..gp) 
TIGERBOT:del(TIGER_ID.."gps:settings:"..gp) 
TIGERBOT:del(TIGER_ID.."gps:settings:"..gp) 
TIGER_sendMsg(text:match('غادر (-%d+)'), 0, 1,'💢*┇*تم مغادره البوت من المجموعه \n🔖*┇*الامر بواسطه مطور البوت \n 💬*┇*للاستفسار راسل مطور البوت ', 1, 'md') 
changeChatMemberStatus(text:match('غادر (-%d+)'), bot_id, "Left") 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'☑*┇*تم ازاله المجموعه من مجموعات البوت بنجاح ✔ ', 1, 'md') 
end 
...
TIGERBOT:set(TIGER_ID..'add:welc:'..msg.chat_id_,'add') 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'☑*┇*  تم تفعيل الترحيب', 1, 'md') 
end
if text ==('تعطيل الترحيب') and is_mod(msg) then 
TIGERBOT:set(TIGER_ID..'add:welc:'..msg.chat_id_,'rem') 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'☑*┇*  تم تعطيل الترحيب', 1, 'md') 
end
if text ==('مسح الترحيب') and is_mod(msg) then 
TIGERBOT:del(TIGER_ID..'welcome:'..msg.chat_id_,welcome) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'☑*┇*  تم مسح ترحيب المجموعه', 1, 'md') 
end
if text and text:match("^ضع صوره") and is_mod(msg) and msg.reply_to_message_id_ == 0 then  
TIGERBOT:set(TIGER_ID..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '💬*┇* ارسل لي الصوره الان ', 1, 'md') 
end           
if text ==('المجموعات') and is_devtaha(msg) then
local list = TIGERBOT:smembers(TIGER_ID..'bot:gpsby:id')  
if #list == 0 then  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*🚫┇ لا توجد مجموعات حاليا *\n', 1, 'md')   
return false  end
local t = '🔖┇* اهلا بك في ايدي المجموعات ※*\n*ٴ⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃*\n'  
for k,v in pairs(list) do   
local nummsg = tonumber(TIGERBOT:get(TIGER_ID..'groupmsg:'..v..':')) 
numrgroup(v) local numg = (TIGERBOT:get(TIGER_ID.."numgrop"..v) or '3')
local namechat = TIGERBOT:get(TIGER_ID..'group:name'..v) 
if namechat then
t = t..'*'..k.."➛* `"..v.."` "..tfgroup(nummsg).."\n*« "..namechat..' » ❪'..numg..'❫*\n*ٴ⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃*\n'   
else
t = t..'*'..k.."➛* `"..v.."` "..tfgroup(nummsg).."\n* ❪"..numg..'❫*\n*ٴ⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃*\n'   
end
file = io.open("TIGER_groups.txt", "w") file:write([[ ]]..isnothtml(t)..[[ ]]) file:close() 
end  
t = t..'*📜┇*لعرض معلومات مجموعه معينه\n*💬┇ ارسل كشف من ثم ايدي المجموعه*\n*🔖┇ مثال ❪كشف -10012345667❫*\n꞉'  
if #list >= 25 then  
local groups = TIGERBOT:scard(TIGER_ID..'bot:gpsby:id')  
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './TIGER_groups.txt','📛┇ عذرا لديك الكثير من المجموعات\n🔖┇ تم ارسال المجموعات في الملف\n💢┇ عدد المجموعات •※'..groups..'※•',dl_cb, nil) 
else 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,t, 1, 'md')  
end 
end
...
local hash = TIGERBOT:get(TIGER_ID..'add:welc:'..msg.chat_id_)     
if hash == 'add' then     
return '✓'     
else     
return '✘'     
...
local hash = TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"flood")     
if hash then      
if TIGERBOT:hget("flooding:settings:"..msg.chat_id_, "flood") == "kick" then     
return 'بالطرد 🚷'     
elseif TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"flood") == "keed" then     
return 'بالتقيد 🔏'     
elseif TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"flood") == "mute" then     
return 'بالكتم 🔇'           
elseif TIGERBOT:hget("flooding:settings:"..msg.chat_id_,"flood") == "del" then     
return 'بالمسح ⚡'     
end      
else     
return '✘'     
...
if TIGERBOT:get(TIGER_ID..'lock:tagservrbot'..msg.chat_id_) then
lock_tagservr_bot = '✓'
else 
lock_tagservr_bot = '✘'    
end
...
if TIGERBOT:get(TIGER_ID..'lock:edit'..msg.chat_id_) then    
lock_edit = '✓'
else 
lock_edit = '✘'    
end
if TIGERBOT:get(TIGER_ID..'lock:edit:media'..msg.chat_id_) then    
lock_edit_med = '✓'
else 
lock_edit_med = '✘'    
end
...
if TIGERBOT:get(TIGER_ID.."lock:Link"..msg.chat_id_) == "del" then
lock_links = '✓'
elseif TIGERBOT:get(TIGER_ID.."lock:Link"..msg.chat_id_) == "ked" then
lock_links = 'بالتقيد 🔏'    
elseif TIGERBOT:get(TIGER_ID.."lock:Link"..msg.chat_id_) == "ktm" then
lock_links = 'بالكتم 🔇'    
elseif TIGERBOT:get(TIGER_ID.."lock:Link"..msg.chat_id_) == "kick" then
lock_links = 'بالطرد 🚷'    
else
lock_links = '✘'    
end
if TIGERBOT:get(TIGER_ID.."lock:Cmd"..msg.chat_id_) == "del" then
lock_cmds = '✓'
elseif TIGERBOT:get(TIGER_ID.."lock:Cmd"..msg.chat_id_) == "ked" then
lock_cmds = 'بالتقيد 🔏'    
elseif TIGERBOT:get(TIGER_ID.."lock:Cmd"..msg.chat_id_) == "ktm" then
lock_cmds = 'بالكتم 🔇'   
elseif TIGERBOT:get(TIGER_ID.."lock:Cmd"..msg.chat_id_) == "kick" then
lock_cmds = 'بالطرد 🚷'    
else
lock_cmds = '✘'    
end
if TIGERBOT:get(TIGER_ID.."lock:user:name"..msg.chat_id_) == "del" then
lock_user = '✓'
elseif TIGERBOT:get(TIGER_ID.."lock:user:name"..msg.chat_id_) == "ked" then
lock_user = 'بالتقيد 🔏'    
elseif TIGERBOT:get(TIGER_ID.."lock:user:name"..msg.chat_id_) == "ktm" then
lock_user = 'بالكتم 🔇'    
elseif TIGERBOT:get(TIGER_ID.."lock:user:name"..msg.chat_id_) == "kick" then
lock_user = 'بالطرد 🚷'    
else
lock_user = '✘'    
end
if TIGERBOT:get(TIGER_ID.."lock:hashtak"..msg.chat_id_) == "del" then
lock_hash = '✓'
elseif TIGERBOT:get(TIGER_ID.."lock:hashtak"..msg.chat_id_) == "ked" then 
lock_hash = 'بالتقيد 🔏'    
elseif TIGERBOT:get(TIGER_ID.."lock:hashtak"..msg.chat_id_) == "ktm" then 
lock_hash = 'بالكتم 🔇'    
elseif TIGERBOT:get(TIGER_ID.."lock:hashtak"..msg.chat_id_) == "kick" then 
lock_hash = 'بالطرد 🚷'    
else
lock_hash = '✘'    
end
...
if TIGERBOT:get(TIGER_ID.."lock:Bot:kick"..msg.chat_id_) == 'del' then
lock_bots = '✓'
elseif TIGERBOT:get(TIGER_ID.."lock:Bot:kick"..msg.chat_id_) == 'ked' then
lock_bots = 'بالتقيد 🔏'   
elseif TIGERBOT:get(TIGER_ID.."lock:Bot:kick"..msg.chat_id_) == 'kick' then
lock_bots = 'بالطرد 🚷'    
else
lock_bots = '✘'    
end
...
if TIGERBOT:get(TIGER_ID.."lock:Spam"..msg.chat_id_) == "del" then    
lock_spam = '✓'
elseif TIGERBOT:get(TIGER_ID.."lock:Spam"..msg.chat_id_) == "ked" then 
lock_spam = 'بالتقيد 🔏'    
elseif TIGERBOT:get(TIGER_ID.."lock:Spam"..msg.chat_id_) == "ktm" then 
lock_spam = 'بالكتم 🔇'    
elseif TIGERBOT:get(TIGER_ID.."lock:Spam"..msg.chat_id_) == "kick" then 
lock_spam = 'بالطرد 🚷'    
else
lock_spam = '✘'    
end        
...
' ❫*\n'..'*✔🙋🏻‍♂️※┇ اشعارات البوتات ↫❪ '..lock_tagservr_bot..
...
' ❫*\n'..'*✔🙋🏻‍♂️※┇ السيلفي ↫❪ '..lock_self..
' ❫*\n'..'*✔🙋🏻‍♂️※┇ الالعاب ↫❪ '..lock_geam..
' ❫*\n'..'*✔🙋🏻‍♂️※┇ التكرار ↫❪ '..getadd_or_rem('spam')..
' ❫*\n'..'*✔🙋🏻‍♂️※┇ الترحيب ↫❪ '..getadd_or_rem('welcome')..
...
TIGERBOT:setex(TIGER_ID.."textch:user" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)  
...
TIGERBOT:del(TIGER_ID..'add:ch:id')
TIGERBOT:del(TIGER_ID..'add:ch:username')
taha = "*☑┇ تم تعطيل الاشتراك الاجباري *\n✓※"
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,taha, 1, 'md') 
end
if text == "الاشتراك الاجباري" and is_devtaha(msg) then  
if TIGERBOT:get(TIGER_ID..'add:ch:username') then
local addchusername = TIGERBOT:get(TIGER_ID..'add:ch:username')
...
if text and text:match('^ضع تكرار (%d+)$') and is_mod(msg) then TIGERBOT:hset("flooding:settings:"..msg.chat_id_ ,"floodmax" ,text:match('ضع تكرار (.*)')) TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'✔🙋🏻‍♂️※*┇*  تم وضع عدد التكرار في المجموعه ( '..text:match('ضع تكرار (.*)')..' )', 1, 'md') end if text and text:match('^ضع زمن التكرار (%d+)$') and is_mod(msg) then TIGERBOT:hset("flooding:settings:"..msg.chat_id_ ,"floodtime" ,text:match('ضع زمن التكرار (.*)')) TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'✔🙋🏻‍♂️※*┇*  تم وضع زمن التكرار في المجموعه ( '..text:match('ضع زمن التكرار (.*)')..' )', 1, 'md') end
if text == 'الترحيب' then 
local getwelcom = TIGERBOT:get(TIGER_ID..'welcome:'..msg.chat_id_)  
if getwelcom then 
...
return false  end
if not TIGERBOT:sismember(TIGER_ID..'vip:group'..msg.chat_id_,result.sender_user_id_) then
if data.username_ == false then
...
TIGERBOT:sadd(TIGER_ID..'vip:group'..msg.chat_id_,result.sender_user_id_)  
else
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ لا تستطيع رفع القناة في البوت *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
...
return false  end
if result.id_ then  
if not TIGERBOT:sismember(TIGER_ID..'vip:group'..msg.chat_id_,result.id_) then
setvip = '*👤┇* العضــو » ❪ [@'..username..'] ❫\n*🔖┇ الايـدي » ❪* `'..result.id_..'` ❫\n*☑┇* تـم تـرقيتـه مميز للـكروب'   
TIGERBOT:sadd(TIGER_ID..'vip:group'..msg.chat_id_,result.id_)  
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🔖┇ لا تستطيع رفع وتنزيل البوتات *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'vip:group'..msg.chat_id_,user) then
if data.username_ == false then
setvip = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n☑┇ تـم تـرقيتـه مميز للـكروب\n✔🙋🏻‍♂️※'   
TIGERmonshn(msg.chat_id_, user, msg.id_, setvip, 16, utf8.len(CatchName(data.first_name_,15)))  
TIGERBOT:sadd(TIGER_ID..'vip:group'..msg.chat_id_,user)  
else
setvip = '*👤┇* العضــو » ❪ [@'..data.username_..'] ❫\n*🔖┇ الايـدي » ❪* `'..user..'` *❫*\n*☑┇* تـم تـرقيتـه مميز للـكروب'   
TIGERBOT:sadd(TIGER_ID..'vip:group'..msg.chat_id_,user)  
seavusername(user) 
...
return false  end
if result and result.type_ and result.type_.channel_ and result.type_.channel_.ID == "Channel" then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🔖┇ لا تستطيع تنزيل القناة في البوت *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if result and result.type_ and result.type_.user_ and result.type_.user_.type_ and result.type_.user_.type_.ID == "UserTypeBot" then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🔖┇ لا تستطيع رفع وتنزيل البوتات *\n??', 1, 'md') 
return false  end
...
local list = TIGERBOT:smembers(TIGER_ID..'mutes'..msg.chat_id_) 
if #list == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*❕┇* لا يوجد مكتومين ليتم مسحهم\n', 1, 'md') 
return false  end
local num = 0
for k,v in pairs(list) do  
TIGERBOT:srem(TIGER_ID..'mutes'..msg.chat_id_,v) 
num = num + 1
end 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*☑┇ تم مسح {'..num..'} من المكتومين *\n', 1, 'md') 
end 
if text ==('مسح المحظورين') and is_mod(msg) then 
local list = TIGERBOT:smembers(TIGER_ID..'TIGER:baned'..msg.chat_id_) 
if #list == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*❕┇* لا يوجد محظورين ليتم مسحهم\n', 1, 'md') 
return false  end
local num = 0
for k,v in pairs(list) do  
TIGERBOT:srem(TIGER_ID..'TIGER:baned'..msg.chat_id_,v) 
num = num + 1
end 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*☑┇ تم مسح {'..num..'} من المحظورين *\n', 1, 'md') 
end 

if text == 'مسح المميزين' and is_mod(msg) then  
local list = TIGERBOT:smembers(TIGER_ID..'vip:group'..msg.chat_id_) 
if #list == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*❕┇* لا يوجد مميزين ليتم مسحهم\n', 1, 'md') 
return false  end
local num = 0
for k,v in pairs(list) do  
TIGERBOT:srem(TIGER_ID..'vip:group'..msg.chat_id_,v) 
num = num + 1
end 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*☑┇ تم مسح {'..num..'} من المميزين *\n', 1, 'md') 
end 
if text ==('مسح الادمنيه') and is_monsh(msg) then 
local list = TIGERBOT:smembers(TIGER_ID..'mods:'..msg.chat_id_) 
if #list == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*❕┇* لا يوجد ادمنيه ليتم مسحهم\n', 1, 'md') 
return false  end
local num = 0
for k,v in pairs(list) do  
TIGERBOT:srem(TIGER_ID..'mods:'..msg.chat_id_,v) 
num = num + 1
end 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*☑┇ تم مسح {'..num..'} من الادمنيه *\n', 1, 'md') 
end
if text ==('مسح المدراء') and is_owner(msg) then 
local list = TIGERBOT:smembers(TIGER_ID..'modergroup'..msg.chat_id_) 
if #list == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*❕┇* لا يوجد مدراء ليتم مسحهم\n', 1, 'md') 
return false  end
local num = 0
for k,v in pairs(list) do  
TIGERBOT:srem(TIGER_ID..'modergroup'..msg.chat_id_,v) 
num = num + 1
end 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*☑┇ تم مسح {'..num..'} من المدراء *\n', 1, 'md') 
end 
if text == 'مسح المنشئين' and is_sudo(msg) then 
local list = TIGERBOT:smembers(TIGER_ID..'moder'..msg.chat_id_) 
if #list == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*┇* لا يوجد منشئين ليتم مسحهم\n', 1, 'md') 
return false  end
local num = 0
for k,v in pairs(list) do  
TIGERBOT:srem(TIGER_ID..'moder'..msg.chat_id_,v) 
num = num + 1
end 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*☑┇ تم مسح {'..num..'} من المنشئين *\n', 1, 'md') 
end

if text == 'مسح المميزين عام' and is_devtaha(msg) then    
local list = TIGERBOT:smembers(TIGER_ID..'vip:groups')  
if #list == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*❕┇* لا يوجد مميزين عام ليتم مسحهم\n', 1, 'md') 
return false  end
local num = 0
for k,v in pairs(list) do  
TIGERBOT:srem(TIGER_ID.."vip:groups",v)  
num = num + 1
end 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*☑┇ تم مسح {'..num..'} من المميزين عام *\n', 1, 'md') 
end
if text == 'مسح المطورين' and is_devtaha(msg) then   
local list = TIGERBOT:smembers(TIGER_ID..'sudo:bot')  
if #list == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*❕┇* لا يوجد مطورين ليتم مسحهم\n', 1, 'md') 
return false  end
local num = 0
for k,v in pairs(list) do  
TIGERBOT:srem(TIGER_ID.."sudo:bot",v)  
num = num + 1
end 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*☑┇ تم مسح {'..num..'} من المطورين *\n', 1, 'md') 
end
if text == 'مسح قائمه العام' and is_devtaha(msg) then 
local list = TIGERBOT:smembers(TIGER_ID..'TIGER:gbaned')  
if #list == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*❕┇* لا يوجد محظورين عام ليتم مسحهم\n', 1, 'md') 
return false  end
local num = 0
for k,v in pairs(list) do  
TIGERBOT:srem(TIGER_ID.."TIGER:gbaned",v)  
num = num + 1
end 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*☑┇ تم مسح {'..num..'} من المحظورين عام *\n', 1, 'md') 
end	 
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🔖┇ لا تستطيع رفع وتنزيل البوتات *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'vip:groups',result.sender_user_id_) then
if data.username_ == false then
...
return false  end
if result.id_ then  
if not TIGERBOT:sismember(TIGER_ID..'vip:groups',result.id_) then
setvips = '*👤┇* العضــو » ❪ [@'..username..'] ❫\n*🔖┇ الايـدي » ❪* `'..result.id_..'` *❫*\n*☑┇* تـم تـرقيتـه مميز عام البوت'   
TIGERBOT:sadd(TIGER_ID..'vip:groups',result.id_)  
TIGERBOT:set(TIGER_ID.."user:Name"..result.id_,"@"..username)
else
setvips = '*👤┇* العضــو » ❪ [@'..username..'] ❫\n*🔖┇ الايـدي » ❪* `'..result.id_..'` *❫*\n*☑┇* بالتاكيد هو مميز عام'   
TIGERBOT:sadd(TIGER_ID..'vip:groups',result.id_)  
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🔖┇ لا تستطيع رفع وتنزيل البوتات *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'vip:groups',user) then
if data.username_ == false then
setvips = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n☑┇ تـم تـرقيتـه مميز عام البوت\n✔🙋🏻‍♂️※'   
TIGERmonshn(msg.chat_id_, user, msg.id_, setvips, 16, utf8.len(CatchName(data.first_name_,15)))  
TIGERBOT:sadd(TIGER_ID..'vip:groups',user)  
else
setvips = '*👤┇* العضــو » ❪ [@'..data.username_..'] ❫\n*🔖┇ الايـدي » ❪* `'..user..'` *❫*\n*☑┇* تـم تـرقيتـه مميز عام البوت'   
TIGERBOT:sadd(TIGER_ID..'vip:groups',user)  
seavusername(user) 
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🔖┇ لا تستطيع رفع وتنزيل البوتات *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'modergroup'..msg.chat_id_,result.sender_user_id_) then
if data.username_ == false then
...
TIGERBOT:sadd(TIGER_ID..'modergroup'..msg.chat_id_,result.sender_user_id_)  
else
zo = '*👤┇* العضــو » ❪ [@'..data.username_..'] ❫\n*🔖┇ الايـدي » ❪* `'..result.sender_user_id_..'` *❫*\n*☑┇* تـم تـرقيتـه مدير للـكروب'   
TIGERBOT:sadd(TIGER_ID..'modergroup'..msg.chat_id_,result.sender_user_id_)  
seavusername(result.sender_user_id_) 
...
return false  end
if result.id_ then  
if not TIGERBOT:sismember(TIGER_ID..'modergroup'..msg.chat_id_,result.id_) then
zo = '*👤┇* العضــو » ❪ [@'..username..'] ❫\n*🔖┇ الايـدي » ❪* `'..result.id_..'` *❫*\n*☑┇* تـم تـرقيتـه مدير للـكروب'   
TIGERBOT:sadd(TIGER_ID..'modergroup'..msg.chat_id_,result.id_)  
TIGERBOT:set(TIGER_ID.."user:Name"..result.id_,"@"..username)
else
zo = '*👤┇* العضــو » ❪ [@'..username..'] ❫\n*🔖┇ الايـدي » ❪* `'..result.id_..'` *❫*\n*☑┇* بالتاكيد هو مدير'   
TIGERBOT:sadd(TIGER_ID..'modergroup'..msg.chat_id_,result.id_)  
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*☑┇ لا تستطيع رفع وتنزيل البوتات *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'modergroup'..msg.chat_id_,user) then
if data.username_ == false then
zo = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n☑┇ تـم تـرقيتـه مدير للـكروب\n✔🙋🏻‍♂️※'   
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🔖┇ لا تستطيع رفع وتنزيل البوتات *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'mods:'..msg.chat_id_,result.sender_user_id_) then
if data.username_ == false then
zo = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n☑┇ تـم تـرقيتـه ادمن للـكروب\n✔🙋🏻‍♂️※'   
TIGERmonshn(msg.chat_id_, result.sender_user_id_, msg.id_, zo, 16, utf8.len(CatchName(data.first_name_,15)))  
TIGERBOT:sadd(TIGER_ID..'mods:'..msg.chat_id_,result.sender_user_id_)  
else
zo = '*👤┇* العضــو » ❪ [@'..data.username_..'] ❫\n*🔖┇ الايـدي » ❪* `'..result.sender_user_id_..'` *❫*\n*☑┇* تـم تـرقيتـه ادمن للـكروب'   
TIGERBOT:sadd(TIGER_ID..'mods:'..msg.chat_id_,result.sender_user_id_)  
seavusername(result.sender_user_id_) 
...
return false  end
if result.id_ then  
if not TIGERBOT:sismember(TIGER_ID..'mods:'..msg.chat_id_,result.id_) then
zo = '*👤┇* العضــو » ❪ [@'..username..'] ❫\n*🔖┇ الايـدي » ❪* `'..result.id_..'` *❫*\n*☑┇* تـم تـرقيتـه ادمن للـكروب'   
TIGERBOT:sadd(TIGER_ID..'mods:'..msg.chat_id_,result.id_)  
TIGERBOT:set(TIGER_ID.."user:Name"..result.id_,"@"..username)
else
zo = '*👤┇* العضــو » ❪ [@'..username..'] ❫\n*🔖┇ الايـدي » ❪* `'..result.id_..'` *❫*\n*☑┇* بالتاكيد هو ادمن'   
TIGERBOT:sadd(TIGER_ID..'mods:'..msg.chat_id_,result.id_)  
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🔖┇ لا تستطيع رفع وتنزيل البوتات *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'mods:'..msg.chat_id_,user) then
if data.username_ == false then
zo = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n☑┇ تـم تـرقيتـه ادمن للـكروب\n✔🙋🏻‍♂️※'   
TIGERmonshn(msg.chat_id_, user, msg.id_, zo, 16, utf8.len(CatchName(data.first_name_,15)))  
TIGERBOT:sadd(TIGER_ID..'mods:'..msg.chat_id_,user)  
else
zo = '*👤┇* العضــو » ❪ [@'..data.username_..'] ❫\n*🔖┇ الايـدي » ❪* `'..user..'` *❫*\n*☑┇* تـم تـرقيتـه ادمن للـكروب'   
TIGERBOT:sadd(TIGER_ID..'mods:'..msg.chat_id_,user)  
seavusername(user) 
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🔖┇ لا تستطيع رفع وتنزيل البوتات *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'moder'..msg.chat_id_,result.sender_user_id_) then
if data.username_ == false then
...
TIGERBOT:sadd(TIGER_ID..'moder'..msg.chat_id_,result.sender_user_id_)  
else
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🔖┇ لا تستطيع رفع وتنزيل البوتات *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'moder'..msg.chat_id_,user) then
if data.username_ == false then
setmonsh = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n☑┇ تـم تـرقيتـه منشئ للـكروب\n✔🙋🏻‍♂️※'   
...
TIGERmonshn(msg.chat_id_, result.sender_user_id_, msg.id_, zo, 16, utf8.len(CatchName(data.first_name_,15)))  
TIGERBOT:sadd(TIGER_ID..'sudo:bot',result.sender_user_id_)  
else
zo = '*👤┇* العضــو » ❪ [@'..data.username_..'] ❫\n*🔖┇ الايـدي » ❪* `'..result.sender_user_id_..'` *❫*\n*☑┇* تـم تـرقيتـه مطور البوت'   
TIGERBOT:sadd(TIGER_ID..'sudo:bot',result.sender_user_id_)  
seavusername(result.sender_user_id_) 
...
return false  end
if result.id_ then  
if not TIGERBOT:sismember(TIGER_ID..'sudo:bot',result.id_) then
zo = '*👤┇* العضــو » ❪ [@'..username..'] ❫\n*🔖┇ الايـدي » ❪* `'..result.id_..'` *❫*\n*☑┇* تـم تـرقيتـه مطور البوت'   
TIGERBOT:sadd(TIGER_ID..'sudo:bot',result.id_)  
TIGERBOT:set(TIGER_ID.."user:Name"..result.id_,"@"..username)
else
zo = '*👤┇* العضــو » ❪ [@'..username..'] ❫\n*🔖┇ الايـدي » ❪* `'..result.id_..'` *❫*\n*☑┇* بالتاكيد هو مطور'   
TIGERBOT:sadd(TIGER_ID..'sudo:bot',result.id_)  
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🔖┇ لا تستطيع رفع وتنزيل البوتات *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'sudo:bot',user) then
if data.username_ == false then
zo = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n☑┇ تـم تـرقيتـه مطور البوت\n✔🙋🏻‍♂️※'   
TIGERmonshn(msg.chat_id_, user, msg.id_, zo, 16, utf8.len(CatchName(data.first_name_,15)))  
TIGERBOT:sadd(TIGER_ID..'sudo:bot',user)  
else
zo = '*👤┇* العضــو » ❪ [@'..data.username_..'] ❫\n*🔖┇ الايـدي » ❪* `'..user..'` *❫*\n*☑┇* تـم تـرقيتـه مطور البوت'   
TIGERBOT:sadd(TIGER_ID..'sudo:bot',user)  
seavusername(user) 
...
if tonumber(result.sender_user_id_) == tonumber(bot_id) then  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🔖┇ انا البوت وليس لدي قيود * \n', 1, 'md')  
return false  end 
tdcli_function ({
ID = "GetUser",
user_id_ = result.sender_user_id_
},function(arg,data) 
if TIGERBOT:sismember(TIGER_ID..'tedmembars'..msg.chat_id_,result.sender_user_id_) or TIGERBOT:sismember(TIGER_ID..'mutes'..msg.chat_id_,result.sender_user_id_) or TIGERBOT:sismember(TIGER_ID..'TIGER:baned'..msg.chat_id_,result.sender_user_id_) then
if data.username_ == false then
zo = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n☑┇ تم رفع قيوده\n✔🙋🏻‍♂️※'   
TIGERmonshn(msg.chat_id_, result.sender_user_id_, msg.id_, zo, 16, utf8.len(CatchName(data.first_name_,15)))  
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ لا استطيع استخراج معلوماته*\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if TIGERBOT:sismember(TIGER_ID..'tedmembars'..msg.chat_id_,user) or TIGERBOT:sismember(TIGER_ID..'mutes'..msg.chat_id_,user) or TIGERBOT:sismember(TIGER_ID..'TIGER:baned'..msg.chat_id_,user) then
if data.username_ == false then
zo = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n☑┇ تم رفع قيوده\n✔🙋🏻‍♂️※'   
TIGERmonshn(msg.chat_id_, user, msg.id_, zo, 16, utf8.len(CatchName(data.first_name_,15)))  
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ لا استطيع استخراج معلوماته*\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if TIGERBOT:sismember(TIGER_ID..'tedmembars'..msg.chat_id_,result.id_) or TIGERBOT:sismember(TIGER_ID..'mutes'..msg.chat_id_,result.id_) or TIGERBOT:sismember(TIGER_ID..'TIGER:baned'..msg.chat_id_,result.id_) then
zo = '*👤┇* العضــو » ❪ [@'..data.username_..'] ❫\n*🔖┇ الايـدي » ❪* `'..result.id_..'` ❫\n*☑┇* تم رفع قيوده\n✔🙋🏻‍♂️※'   
...
function unbanreply(extra, result, success) 
if tonumber(result.sender_user_id_) == tonumber(bot_id) then  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ انا البوت ولست محظور * \n', 1, 'md')  
return false  end 
tdcli_function ({
ID = "GetUser",
user_id_ = result.sender_user_id_
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ يرجى تفعيل صلاحيه حظر المستخدمين لاتمكن من الغاء حظره *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
...
return false  end
if result and result.type_ and result.type_.channel_ and result.type_.channel_.ID == "Channel" then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ لا تستطيع حظر القناة *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if tonumber(result.id_) == tonumber(bot_id) then  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ انا البوت ولست محظور * \n', 1, 'md')  
return false  end 
...
for k, v in pairs(list) do 
local taha = TIGERBOT:get(TIGER_ID.."user:Name" .. v)
if taha then
local username = taha
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ انا لست ادمن في المجموعه *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'TIGER:gbaned',result.sender_user_id_) then
if data.username_ == false then
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ عذرآ لآ أستطيع حظر » ❪ '..get_mokforkick(msg.chat_id_,result.id_)..' ❫ *\n', 1, 'md') 
return false  end 
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ عذرآ لآ أستطيع حظر » ❪ '..get_mokforkick(msg.chat_id_,user)..' ❫ *\n', 1, 'md') 
return false  end 
tdcli_function ({
ID = "GetUser",
user_id_ = user
...
return false  end
if not TIGERBOT:sismember(TIGER_ID..'TIGER:gbaned',user) then
if data.username_ == false then
zo = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n🚫┇ تـم حظره عام من الكروبات\n✔🙋🏻‍♂️※'   
TIGERmonshn(msg.chat_id_, user, msg.id_, zo, 16, utf8.len(CatchName(data.first_name_,15)))  
local list = TIGERBOT:smembers(TIGER_ID..'bot:gpsby:id')  for k,v in pairs(list) do  kick(msg,v,user) end
...
if ban and ban.code_ and ban.code_ == 400 and ban.message_ == "CHAT_ADMIN_REQUIRED" then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ يرجى تفعيل صلاحيه حظر المستخدمين لاتمكن من الغاء حظره عام *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if ban and ban.code_ and ban.code_ == 3 then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ انا لست ادمن في المجموعه *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
...
function prom_reply(extra, result, success) 
if tonumber(result.sender_user_id_) == tonumber(bot_id) then  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ عذرآ لأ أستطيع كتم نفسي * ', 1, 'md')  
return false  end 
if TIGER1(msg.chat_id_,result.sender_user_id_) then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ عذرآ لآ أستطيع كتم » ❪ '..get_mokforkick(msg.chat_id_,result.sender_user_id_)..' ❫ *\n', 1, 'md') 
return false  end 
tdcli_function ({
ID = "GetUser",
user_id_ = result.sender_user_id_
},function(arg,data) 
if not TIGERBOT:sismember(TIGER_ID..'mutes'..msg.chat_id_,result.sender_user_id_) then
if data.username_ == false then
zo = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n🚫┇ تـم كتمه من الكروب\n✔🙋🏻‍♂️※'   
TIGERmonshn(msg.chat_id_, result.sender_user_id_, msg.id_, zo, 16, utf8.len(CatchName(data.first_name_,15)))   
...
return false  end 
if TIGER1(msg.chat_id_,user) then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ عذرآ لآ أستطيع كتم » ❪ '..get_mokforkick(msg.chat_id_,user)..' ❫ *\n', 1, 'md') 
return false  end 
tdcli_function ({
ID = "GetUser",
user_id_ = user
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ لا استطيع كتم ادمنية المجموعه *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'mutes'..msg.chat_id_,user) then
if data.username_ == false then
zo = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n🚫┇ تـم كتمه من الكروب\n✔🙋🏻‍♂️※'   
TIGERmonshn(msg.chat_id_, user, msg.id_, zo, 16, utf8.len(CatchName(data.first_name_,15)))  
...
function unbanreply(extra, result, success) 
if tonumber(result.sender_user_id_) == tonumber(bot_id) then  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ انا البوت ولست مكتوم * \n', 1, 'md')  
return false  end 
tdcli_function ({
ID = "GetUser",
user_id_ = result.sender_user_id_
...
if ban and ban.code_ and ban.code_ == 400 and ban.message_ == "USER_ADMIN_INVALID" then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ لا استطيع حظر ادمنية المجموعه *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if ban and ban.code_ and ban.code_ == 400 and ban.message_ == "CHAT_ADMIN_REQUIRED" then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ ليس لدي صلاحية حظر المستخدمين *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if ban and ban.code_ and ban.code_ == 3 then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ انا لست ادمن في المجموعه *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'TIGER:baned'..msg.chat_id_,result.sender_user_id_) then
if data.username_ == false then
zo = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n🚫┇ تـم حظره من الكروب\n✔🙋🏻‍♂️※'   
TIGERmonshn(msg.chat_id_, result.sender_user_id_, msg.id_, zo, 16, utf8.len(CatchName(data.first_name_,15)))   
...
TIGER_send(msg.chat_id_,msg.id_,"*❕┇ انت لست ادمن \n👨🏻‍✈️*")   
end
end
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ لا استطيع استخراج معلوماته*\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not TIGERBOT:sismember(TIGER_ID..'TIGER:baned'..msg.chat_id_,user) then
if data.username_ == false then
zo = '👤┇ العضــو » ❪ '..CatchName(data.first_name_,15)..' ❫\n📬┇ تـم حظره من الكروب\n✔🙋🏻‍♂️※'   
TIGERmonshn(msg.chat_id_, user, msg.id_, zo, 16, utf8.len(CatchName(data.first_name_,15)))  
...
return false  end
if result and result.type_ and result.type_.channel_ and result.type_.channel_.ID == "Channel" then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*📮┇ لا تستطيع طرد القناة *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if tonumber(result.id_) == tonumber(bot_id) then  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ عذرآ لأ أستطيع طرد نفسي * \n', 1, 'md')  
return false  end 
...
return false  end
if TIGER1(msg.chat_id_,result.id_) then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ عذرآ لآ أستطيع طرد » ❪ '..get_mokforkick(msg.chat_id_,result.id_)..' ❫ *\n', 1, 'md') 
return false  end 
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = msg.chat_id_, user_id_ = result.id_, status_ = { ID = "ChatMemberStatusKicked" }, 
},function(arg,ban) 
if ban and ban.code_ and ban.code_ == 400 and ban.message_ == "USER_ADMIN_INVALID" then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ لا استطيع طرد ادمنية المجموعه *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if ban and ban.code_ and ban.code_ == 400 and ban.message_ == "CHAT_ADMIN_REQUIRED" then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ ليس لدي صلاحية حظر المستخدمين *\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
...
return false  end 
if TIGER1(msg.chat_id_,result.sender_user_id_) then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ عذرآ لآ أستطيع طرد » ❪ '..get_mokforkick(msg.chat_id_,result.sender_user_id_)..' ❫ *\n', 1, 'md') 
return false  end 
tdcli_function ({ID = "GetChatMember",chat_id_ = msg.chat_id_,user_id_ = result.sender_user_id_},function(arg,da) 
tdcli_function ({
ID = "GetUser",
...
return false  end 
if TIGER1(msg.chat_id_,user) then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ عذرآ لآ أستطيع طرد » ❪ '..get_mokforkick(msg.chat_id_,user)..' ❫ *\n', 1, 'md') 
return false  end 
...
if taha > 100 then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇ تستطيح المسح فقط (100) رساله*\n', 1, 'md') 
else 
function cb(a,b,c) 
...
TIGER_sendMsg(msg.chat_id_,msg.id_, 1, "*❕┇* لا استطيع المسح انا لست ادمن \n", 1, 'md')  
return false  end 
end,nil) 
end
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'☑*┇ تم مسح {'..b.total_count_..'} رساله فقط* \n', 1, 'md')  
end 
...
tdcli_function ({ID="DeleteMessages", chat_id_=msg.chat_id_,  message_ids_={[0] = tonumber(msg.reply_to_message_id_),msg.id_} },function(arg,data)  if data.code_ == 6 then TIGER_sendMsg(msg.chat_id_,msg.id_, 1, "*📮┇* انا لست ادمن او ليست لدي صلاحيه حذف الرسائل \n✔🙋🏻‍♂️※", 1, 'md')   return false  end  end,nil) 
end	 
end
if text and text:match('^منع (.*)') and is_mod(msg) then 
local taha = text:match('^منع (.*)') 
if TIGERBOT:sismember(TIGER_ID..'filters:'..msg.chat_id_,taha) then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*📜┇ الكلمه » ❪ '..taha..' ❫*\n*☑┇* بالتأكيد تم منع هاذه الكلمه \n✔🙋🏻‍♂️※', 1, 'md') 
else
TIGERBOT:sadd(TIGER_ID..'filters:'..msg.chat_id_,taha) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*📜┇ الكلمه » ❪ '..taha..' ❫*\n*☑┇* تم منع ارسال هاذه الكلمه \n✓※', 1, 'md') 
end
end 
if text and text:match('^الغاء المنع (.*)')  and is_mod(msg) then  
local taha = text:match('^الغاء المنع (.*)') 
if not TIGERBOT:sismember(TIGER_ID..'filters:'..msg.chat_id_,taha) then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*⚜┇ الكلمه » ❪ '..taha..' ❫*\n*🎴┇* بالتأكيد تم الغاء منع الكلمه\n✔🙋🏻‍♂️※', 1, 'md') 
else
TIGERBOT:srem(TIGER_ID..'filters:'..msg.chat_id_,taha) 
...
if text ==('مسح قائمه المنع') and is_mod(msg) then TIGERBOT:del(TIGER_ID..'filters:'..msg.chat_id_) TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'📜*┇* تم مسح قائمه المنع', 1, 'md') end 
if text == ('قائمه المنع') and is_mod(msg) then  
local list = TIGERBOT:smembers(TIGER_ID..'filters:'..msg.chat_id_) 
...
TIGER_sendMsg(msg.chat_id_,msg.id_, 1, "*📌┇* بالتاكيد تم تثبيت الرساله \n✔🙋🏻‍♂️※", 1, 'md')  
elseif data.code_ == 6 then
TIGER_sendMsg(msg.chat_id_,msg.id_, 1, "*❕┇* لا استطيع تثبيت الرساله انا لست ادمن \n✘※", 1, 'md')  
...
if text == 'اطردني' and not TIGERBOT:get(TIGER_ID.."lock:kickme"..msg.chat_id_)   then 
tdcli_function ({
ID = "GetChatMember",
chat_id_ = msg.chat_id_,
user_id_ = msg.sender_user_id_},
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '❔*┇* لا استطيع طردك انته منشئ المجموعه\n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if data.status_.ID == "ChatMemberStatusEditor" then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '❔*┇* لا استطيع طردك انت ادمن في المجموعه \n✔🙋🏻‍♂️※', 1, 'md') 
return false  end
if not msg.can_be_deleted_ == true then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*❔┇* لا استطيع الطرد انا لست ادمن هنا \n✔🙋🏻‍♂️※", 1, "md") 
TIGERBOT:del(TIGER_ID..'kick:'..msg.chat_id_..':'..msg.sender_user_id_, true) 
return false  end 
if TIGER1(msg.chat_id_,msg.sender_user_id_) then 
if tonumber(msg.sender_user_id_) == tonumber(SUDO) then
t = 'المطور'
elseif TIGERBOT:sismember(TIGER_ID..'sudo:bot',msg.sender_user_id_) then
t = 'المطور'
elseif TIGERBOT:sismember(TIGER_ID..'moder'..msg.chat_id_,msg.sender_user_id_) then
t = 'المنشئ'
elseif TIGERBOT:sismember(TIGER_ID..'modergroup'..msg.chat_id_,msg.sender_user_id_) then
t = 'المدير'
elseif TIGERBOT:sismember(TIGER_ID..'mods:'..msg.chat_id_,msg.sender_user_id_) then
t = 'الادمن'
elseif TIGERBOT:sismember(TIGER_ID..'vip:groups',msg.sender_user_id_) then
t = 'مميز عام'
elseif TIGERBOT:sismember(TIGER_ID..'vip:group'..msg.chat_id_,msg.sender_user_id_) then
t = 'مميز'
else
t = ' '
end
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❔┇* عذرا انت ❪ '..get_mok(msg.chat_id_,msg.sender_user_id_)..' ❫ لا استطيع طردك \n✔🙋🏻‍♂️※', 1, 'md') 
TIGERBOT:del(TIGER_ID..'kick:'..msg.chat_id_..':'..msg.sender_user_id_, true) 
return false  end  
local hash = 'kick:'..msg.chat_id_..':'..msg.sender_user_id_ 
TIGERBOT:set(TIGER_ID..hash, "waite") 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '💢*┇* رائع الان يمكنك تطبيق الشروط\n⚀*┇* ارسل ❪ نعم ❫ ليتم طردك\n*⚁┇* ارسل ❪ لا ❫ لالغاء طردك\n✔🙋🏻‍♂️※', 1, 'md') 
end,nil)
end 
if text then 
local hash = 'kick:'..msg.chat_id_..':'..msg.sender_user_id_ 
if text:match("^نعم$") and TIGERBOT:get(TIGER_ID..hash) == "waite" then 
TIGERBOT:set(TIGER_ID..hash, "نعم") 
elseif text:match("^لا$") and TIGERBOT:get(TIGER_ID..hash) == "waite" then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🎴┇* حسنآ لن يتم طردك من هنا\n💭', 1, 'md') 
TIGERBOT:del(TIGER_ID..hash, true) 
end 
end 
local hash = 'kick:'..msg.chat_id_..':'..msg.sender_user_id_ 
if TIGERBOT:get(TIGER_ID..hash) then 
if TIGERBOT:get(TIGER_ID..hash) == "نعم" then 
if TIGER1(msg.chat_id_,msg.sender_user_id_) then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🎴┇* عذرا انت ❪ '..get_mok(msg.chat_id_,msg.sender_user_id_)..' ❫ لا استطيع طردك \n✔🙋🏻‍♂️※', 1, 'md') 
else  
kick(msg,msg.chat_id_,msg.sender_user_id_)  
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*☑┇* تم طردك من هنا \n✔🙋🏻‍♂️※', 1, 'md') 
end 
TIGERBOT:del(TIGER_ID..'kick:'..msg.chat_id_..':'..msg.sender_user_id_, true) 
end 
end
if text == 'نزلني' and not TIGERBOT:get(TIGER_ID.."lock:demrtpa"..msg.chat_id_)   then 
local hash = 'demrtpa:'..msg.chat_id_..':'..msg.sender_user_id_ TIGERBOT:set(TIGER_ID..hash, "waite") 
if TIGER2(msg.chat_id_,msg.sender_user_id_) then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*❕┇* انت من ضمن ❪ '..get_mok2(msg.chat_id_,msg.sender_user_id_)..' ❫ ليس من صلاحياتي تنزيلك \n⭐', 1, 'md') 
TIGERBOT:del(TIGER_ID..'demrtpa:'..msg.chat_id_..':'..msg.sender_user_id_, true)  
else  
if not is_memar(msg)  then  
TIGERBOT:del(TIGER_ID..'demrtpa:'..msg.chat_id_..':'..msg.sender_user_id_, true) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*🎴┇* انت هنا لست سواء عضو ليس لديك رتبه في البوت\n✔🙋🏻‍♂️※', 1, 'md') 
TIGERBOT:del(TIGER_ID..'demrtpa:'..msg.chat_id_..':'..msg.sender_user_id_, true)  
else 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '📛*┇* اهلا بك عزيزي ❪ '..get_mok3(msg.chat_id_,msg.sender_user_id_)..' ❫✔🙋🏻‍♂️※\n📮*┇* رائع الان يمكنك تطبيق الشروط\n⚀*┇* ارسل ❪ نعم ❫ ليتم تنزيلك\n*⚁┇* ارسل ❪ لا ❫ لالغاء تنزيلك\n✔🙋🏻‍♂️※', 1, 'md') 
end 
end 
end 
if text then 
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '*💢┇* لن يتم تنزيلك ستبقى ضمن ❪ '..t..' ❫\n✔🙋🏻‍♂️※', 1, 'md') 
end 
end 
local hash = 'demrtpa:'..msg.chat_id_..':'..msg.sender_user_id_ 
if TIGERBOT:get(TIGER_ID..hash) then 
...
if not msg.can_be_deleted_ == true then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*🚨┇* تنبيــه انا لست ادمن يرجى ترقيتي ادمن هنا 👮\n", 1, "md") 
else  
TIGERBOT:set(TIGER_ID..'add:mepr:'..msg.chat_id_,true) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,"☑❯ تم تفعيل الاضافه اجباري", 1, 'md') 
end 
end
if text == "تعطيل الاضافه" and is_owner(msg) then 
TIGERBOT:del(TIGER_ID..'add:mepr:'..msg.chat_id_) 
TIGERBOT:del(TIGER_ID..'meaddwy:'..msg.chat_id_) 
TIGERBOT:del(TIGER_ID..'meaddtaha:'..msg.chat_id_) 
local taha = TIGERBOT:keys("addedtaha:*") 
for i=1,#taha do TIGERBOT:del(TIGER_ID..taha[i]) 
end 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,"☑❯ تم تعطيل الاضافه اجباري", 1, 'md') 
end
if text and text:match("^ضع عدد الاضافه (%d+)$") and is_owner(msg) then 
local numadded = string.match(text, "^ضع عدد الاضافه (%d+)$") 
TIGERBOT:set(TIGER_ID..'setadd:'..msg.chat_id_,numadded) 
TIGER_sendMsg(msg.chat_id_, msg.id_,  1, "☑❯ تم وضع عدد الاضافه *{ "..numadded..' }*', 1, 'md') 
end 
...
local nummsg = tonumber(TIGERBOT:get(TIGER_ID..'groupmsg:'..msg.chat_id_..':')) 
local nummsgg = TIGERBOT:get(TIGER_ID..'groupmsg:'..msg.chat_id_..':') 
...
if text and text:match("^مشاهده منشور$") then TIGERBOT:set(TIGER_ID..'viewchannel'..msg.sender_user_id_,true) TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '♼*┇* حسننا ارسل التوجيه من القناة سارسل عدد المشاهدات', 1, 'md') end 
if text and text:match("^ضع رد تواصل$") and is_devtaha(msg) then   TIGERBOT:setex(TIGER_ID.."sudo:pv" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)  TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'📬*┇* ارسل لي النص الذي تريده ', 1, 'md') end 
if text and text:match("^ضع كليشه المطور$") and is_devtaha(msg) then TIGERBOT:setex(TIGER_ID.."sudo:dev" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)  TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'📬*┇* ارسل لي النص الذي تريده ', 1, 'md') end 
if text and text:match("^ضع كليشه ستارت$") and is_devtaha(msg) then TIGERBOT:setex(TIGER_ID.."start:msgofstart" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 10000, true)  TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'📬*┇* ارسل لي النص الذي تريده ', 1, 'md') end 
if msg.content_.ID == 'MessageChatChangeTitle' then tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,data)  TIGERBOT:set(TIGER_ID..'group:name'..msg.chat_id_,data.title_) end,nil)  end
...
TIGER_sendMsg(msg.chat_id_,msg.id_, 1, "*🔖️┇* تم وضع اسم للمجموعه \n", 1, 'md')  
TIGERBOT:set(TIGER_ID..'group:name'..result.chat_id_,result.content_.text_)
end
end,nil) 
end 
...
if data.code_ == 3 then
TIGER_sendMsg(result.chat_id_, result.id_, 1, '*❕┇* عذرا انا لست ادمن هنا \n', 1, 'md') 
return false  end
if data.message_ == "CHAT_ADMIN_REQUIRED" then 
TIGER_sendMsg(result.chat_id_, result.id_, 1, '*❔┇* عذرا ليست لدي صلاحيه تغير معلومات المجموعه \n', 1, 'md') 
else
TIGER_sendMsg(result.chat_id_, result.id_, 1, '*🎆┇* تم تغيير صورة المجموعه \n✓※', 1, 'md') 
end
end, nil) 
end   
end 
...
if TIGERBOT:get(TIGER_ID..'lock:rep:rd'..msg.chat_id_) then
taha = '*🗳┇ تم تفعيل الردود بالرد *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:del(TIGER_ID..'lock:rep:rd'..msg.chat_id_)  
else
...
if not TIGERBOT:get(TIGER_ID..'lock:rep:rd'..msg.chat_id_) then
taha = '*🗳┇ تم تعطيل الردود بالرد *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:set(TIGER_ID..'lock:rep:rd'..msg.chat_id_,true)   
else
...
if TIGERBOT:get(TIGER_ID..'lock:rep'..msg.chat_id_) then
taha = '*🚦┇ تم تفعيل الردود *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:del(TIGER_ID..'lock:rep'..msg.chat_id_)  
else
...
if not TIGERBOT:get(TIGER_ID..'lock:rep'..msg.chat_id_) then
taha = '*🚦┇ تم تعطيل الردود *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:set(TIGER_ID..'lock:rep'..msg.chat_id_,true)   
else
...
if TIGERBOT:get(TIGER_ID..'lock:id'..msg.chat_id_)  then
taha = '*🎫┇ تم تفعيل الايدي *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:del(TIGER_ID..'lock:id'..msg.chat_id_) 
else
...
if not TIGERBOT:get(TIGER_ID..'lock:id'..msg.chat_id_)  then
taha = '*🎫┇ تم تعطيل الايدي *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:set(TIGER_ID..'lock:id'..msg.chat_id_,true) 
else
...
if TIGERBOT:get(TIGER_ID..'lock:id:photo'..msg.chat_id_)  then
taha = '*💢┇ تم تفعيل الايدي *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:del(TIGER_ID..'lock:id:photo'..msg.chat_id_) 
TIGERBOT:del(TIGER_ID..'lock:id'..msg.chat_id_) 
else
taha = '*💢┇ بالتاكيد تم تفعيل الايدي  *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
...
if not TIGERBOT:get(TIGER_ID..'lock:id:photo'..msg.chat_id_)  then
taha = '*☑┇ تم تعطيل الايدي بالصوره *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:set(TIGER_ID..'lock:id:photo'..msg.chat_id_,true) 
TIGERBOT:set(TIGER_ID..'lock:id'..msg.chat_id_,true) 
else
...
if TIGERBOT:get(TIGER_ID..'lock:get:photo'..msg.chat_id_) then
taha = '*🔖️┇ تم تفعيل امر جلب الصور *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:del(TIGER_ID..'lock:get:photo'..msg.chat_id_) 
else
...
if not TIGERBOT:get(TIGER_ID..'lock:get:photo'..msg.chat_id_) then
taha = '*🔖️┇ تم تعطيل امر جلب الصور *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:set(TIGER_ID..'lock:get:photo'..msg.chat_id_,true) 
else
...
if TIGERBOT:get(TIGER_ID..'lock:kickme'..msg.chat_id_)  then
taha = '*🀄┇ تم تفعيل امر اطردني *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:del(TIGER_ID..'lock:kickme'..msg.chat_id_) 
else
...
if not TIGERBOT:get(TIGER_ID..'lock:kickme'..msg.chat_id_)  then
taha = '*🀄┇ تم تعطيل امر اطردني *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:set(TIGER_ID..'lock:kickme'..msg.chat_id_,true) 
else
...
if TIGERBOT:get(TIGER_ID..'lock:demrtpa'..msg.chat_id_) then
taha = '*🎴┇ تم تفعيل امر نزلني *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:del(TIGER_ID..'lock:demrtpa'..msg.chat_id_) 
else
...
if not TIGERBOT:get(TIGER_ID..'lock:demrtpa'..msg.chat_id_) then
taha = '*🎴┇ تم تعطيل امر نزلني *\n✓※' 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, taha, 1, "md") 
TIGERBOT:set(TIGER_ID..'lock:demrtpa'..msg.chat_id_,true) 
else
...
if text and text:match("^صورتي (%d+)") and not TIGERBOT:get(TIGER_ID.."lock:get:photo"..msg.chat_id_)  then 
...
local dev = TIGERBOT:get(TIGER_ID.."dev") 
if dev then 
TIGER_sendMsg(msg.chat_id_, msg.id_,1,''..check_markdown(dev)..'',1,'md') 
else 
TIGER_sendMsg(msg.chat_id_, msg.id_,1,'💢*┇* عذرا لم يتم وضع كليشه المطور ',1,'md')  
end  
end
if text == "حذف كليشه المطور" and is_devtaha(msg) then  
TIGERBOT:del(TIGER_ID.."dev") 
TIGER_sendMsg( msg.chat_id_, msg.id_, 1, '*☑┇*تم حذف كليشه المطور', 1, "md") 
end 
if text == "جلب رد التواصل" and is_devtaha(msg) then 
if TIGERBOT:get(TIGER_ID.."pvstart") then
...
local msgtaha = 500 
local msggstaha = TIGERBOT:get(TIGER_ID..'user:messages:'..msg.chat_id_..':'..msg.sender_user_id_) or 0  
local nupmsgg =  tonumber((msggstaha / msgtaha) * 100)   
local msguser = tonumber(TIGERBOT:get(TIGER_ID..'msg:user:'..msg.chat_id_..':'..msg.sender_user_id_) or 1) 
local tahamsg = (TIGERBOT:get(TIGER_ID..'user:messages:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)
if is_devtaha(msg) then 
t = 'مطور اساسي 🌟'  
elseif is_sudo(msg) then 
t = 'مطور البوت ✨'  
...
return false  end 
local zo = '🔖 ※ رتبته البوت » { '..get_rtpa(msg.chat_id_,result.sender_user_id_)..' }\n📓※ رتبته الكروب » { '..rtpa..' }*\n'
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,zo, 1, 'md') 
end,nil)
end
...
return false  end 
local zo = '🔖※ رتبته البوت » { '..get_rtpa(msg.chat_id_,result.id_)..' }\n📓※ رتبته الكروب » { '..rtpa..' }*\n'
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,zo, 1, 'md') 
end,nil)
end
...
return false  end 
local zo = '🔖 ※ رتبته البوت » { '..get_rtpa(msg.chat_id_,userid)..' }\n📓※ رتبته الكروب » { '..rtpa..' }*\n'
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,zo, 1, 'md') 
end,nil)
end
if text == "تفاعلي" then
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,tahaa, 1, 'md') 
end
end
if text ==('رتبتي')  then
tdcli_function ({ID = "GetChatMember",chat_id_ = msg.chat_id_,user_id_ = msg.sender_user_id_},function(arg,da) 
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,tahaid, 1, 'md')
return false  
end 
if text == 'مسح تغير الايدي' and is_devtaha(msg) or text == 'حذف تغير الايدي' and is_devtaha(msg) then
TIGERBOT:del(TIGER_ID.."KLISH:ID")
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,  '*☑┇ تم مسح كليشه الايدي *\n', 1, 'md')
return false  
end 
if TIGERBOT:get(TIGER_ID.."CHENG:ID"..msg.chat_id_..""..msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, "*☑┇ تم الغاء الامر *\n✓※", 1, "md") 
TIGERBOT:del(TIGER_ID.."CHENG:ID"..msg.chat_id_..""..msg.sender_user_id_) 
return false  end 
TIGERBOT:del(TIGER_ID.."CHENG:ID"..msg.chat_id_..""..msg.sender_user_id_) 
...
TIGERBOT:set(TIGER_ID.."KLISH:ID",CHENGER_ID)
...
if not TIGERBOT:get(TIGER_ID..'lock:id'..msg.chat_id_) then      
local get_id_text = TIGERBOT:get(TIGER_ID.."KLISH:ID")
if get_id_text then
...
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,get_id(msg,taha,get_id_text)..'\n• لا يمكنني عرض صورتك لانك قمت بحظر البوت\n', 1, 'html')   
else
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '\n• ليس لديك صور في حسابك ✔🙋🏻‍♂️※\n'..get_id(msg,taha,get_id_text),1, 'html')      
end 
end
else
if result.status_.ID == "UserStatusRecently" and result.profile_photo_ ~= false then
...
local get_id_text = TIGERBOT:get(TIGER_ID.."KLISH:ID")
if get_id_text then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,get_id(msg,taha,get_id_text), 1, 'html')   
else
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'🎟 ※ ايديك ✓ '..msg.sender_user_id_..''..usernamee..'\n🛰 ※ موقعك ✓ '..get_rtpa(msg.chat_id_,msg.sender_user_id_)..'\n📓 ※ موقع الكروب ✓ '..rtpa..'\n💌 ※ تفاعلك ✓ '..formsgg(msguser)..'\n📖 ※ رسائلك ✓ '..(msguser)..'\n📱※ جهاتك ✓ '..(addmempar)..'', 1, 'md')   
end
end
end
end,nil)
...
if TIGERBOT:sismember(TIGER_ID..'modergroup'..msg.chat_id_,result.id_) then
tt = 'مدير'
elseif TIGERBOT:sismember(TIGER_ID..'mods:'..msg.chat_id_,result.id_) then
tt = 'ادمن'
else
tt = 'عضو'
end
...
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'LOCK:PINMSG'..msg.chat_id_,result.sender_user_id_)     
...
if (result and result.type_ and result.type_.ID == "ChannelChatInfo") then
TIGER_send(msg.chat_id_,msg.id_,"*📮┇ هاذا معرف قناة \n👨🏻‍✈️*")   
return false 
end      
TIGERBOT:srem(TIGER_ID..'LOCK:PINMSG'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم الغاء منع التثبيت عنه \n',result.id_)   
end    
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:sadd(TIGER_ID..'LOCK:STEKR'..msg.chat_id_,result.sender_user_id_)     
...
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'LOCK:STEKR'..msg.chat_id_,result.sender_user_id_)     
...
TIGERBOT:srem(TIGER_ID..'LOCK:STEKR'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم الغاء منع الملصقات عنه\n',result.id_)   
end    
tdcli_function ({ID = "SearchPublicChat",username_ = username},py_username,nil) 
end
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:sadd(TIGER_ID..'LOCK:PHOTO'..msg.chat_id_,result.sender_user_id_)     
...
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'LOCK:PHOTO'..msg.chat_id_,result.sender_user_id_)     
...
TIGERBOT:srem(TIGER_ID..'LOCK:PHOTO'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم الغاء منع الصور عنه\n',result.id_)   
end    
tdcli_function ({ID = "SearchPublicChat",username_ = username},py_username,nil) 
end
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:sadd(TIGER_ID..'LOCK:INLIN'..msg.chat_id_,result.sender_user_id_)     
...
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'LOCK:INLIN'..msg.chat_id_,result.sender_user_id_)     
...
TIGERBOT:srem(TIGER_ID..'LOCK:INLIN'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم الغاء منع الاونلاين عنه \n',result.id_)   
end    
tdcli_function ({ID = "SearchPublicChat",username_ = username},py_username,nil) 
end
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:sadd(TIGER_ID..'LOCK:FWD'..msg.chat_id_,result.sender_user_id_)     
...
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'LOCK:FWD'..msg.chat_id_,result.sender_user_id_)     
...
TIGERBOT:srem(TIGER_ID..'LOCK:FWD'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم الغاء منع التوجيه عنه \n',result.id_)   
end    
tdcli_function ({ID = "SearchPublicChat",username_ = username},py_username,nil) 
end
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:sadd(TIGER_ID..'LOCK:LINKS'..msg.chat_id_,result.sender_user_id_)     
...
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'LOCK:LINKS'..msg.chat_id_,result.sender_user_id_)     
...
TIGERBOT:srem(TIGER_ID..'LOCK:LINKS'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم الغاء منع الروابط عنه \n',result.id_)   
end    
tdcli_function ({ID = "SearchPublicChat",username_ = username},py_username,nil) 
end
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:sadd(TIGER_ID..'LOCK:SELPHY'..msg.chat_id_,result.sender_user_id_)     
...
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'LOCK:SELPHY'..msg.chat_id_,result.sender_user_id_)     
...
TIGERBOT:srem(TIGER_ID..'LOCK:SELPHY'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم الغاء منع السيلفي عنه \n',result.id_)   
end    
tdcli_function ({ID = "SearchPublicChat",username_ = username},py_username,nil) 
end
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:sadd(TIGER_ID..'LOCK:VICO'..msg.chat_id_,result.sender_user_id_)     
...
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'LOCK:VICO'..msg.chat_id_,result.sender_user_id_)     
...
TIGERBOT:srem(TIGER_ID..'LOCK:VICO'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم الغاء منع الصوت عنه \n',result.id_)   
end    
tdcli_function ({ID = "SearchPublicChat",username_ = username},py_username,nil) 
end
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:sadd(TIGER_ID..'LOCK:VIDEO'..msg.chat_id_,result.sender_user_id_)     
...
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'LOCK:VIDEO'..msg.chat_id_,result.sender_user_id_)     
...
TIGERBOT:srem(TIGER_ID..'LOCK:VIDEO'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم الغاء منع الفيديو عنه \n',result.id_)   
end    
tdcli_function ({ID = "SearchPublicChat",username_ = username},py_username,nil) 
end
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:sadd(TIGER_ID..'LOCK:MARKDWN'..msg.chat_id_,result.sender_user_id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(data.first_name_)..'}'..'\n☑┇ تم منعه من ارسال الماركدون هنا\n',result.sender_user_id_)   
end,nil)   
end   
tdcli_function ({ ID = "GetMessage", chat_id_ = msg.chat_id_, message_id_ = tonumber(msg.reply_to_message_id_) }, by_reply, nil)
end
if text and text:match('^قفل الماركدون @(.*)') and is_owner(msg) then   
local user = text:match('قفل الماركدون @(.*)')    
function py_username(extra, result, success)   
...
----------------------------------------------
if text == 'فتح الماركدون' and tonumber(msg.reply_to_message_id_) > 0 and is_owner(msg) then   
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'LOCK:MARKDWN'..msg.chat_id_,result.sender_user_id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(data.first_name_)..'}'..'\n☑┇ تم الغاء منع الماركدون عنه \n',result.sender_user_id_)   
end,nil)   
end   
tdcli_function ({ ID = "GetMessage", chat_id_ = msg.chat_id_, message_id_ = tonumber(msg.reply_to_message_id_) }, by_reply, nil)
end
if text and text:match('^فتح الماركدون @(.*)') and is_owner(msg) then   
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:sadd(TIGER_ID..'LOCK:GIF'..msg.chat_id_,result.sender_user_id_)     
...
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'LOCK:GIF'..msg.chat_id_,result.sender_user_id_)     
...
TIGERBOT:srem(TIGER_ID..'LOCK:GIF'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم الغاء منع المتحركه عنه \n',result.id_)   
end    
tdcli_function ({ID = "SearchPublicChat",username_ = username},py_username,nil) 
end
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:sadd(TIGER_ID..'LOCK:USERNAME'..msg.chat_id_,result.sender_user_id_)     
...
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'LOCK:USERNAME'..msg.chat_id_,result.sender_user_id_)     
...
TIGERBOT:srem(TIGER_ID..'LOCK:USERNAME'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم الغاء منع المعرفات عنه \n',result.id_)   
end    
tdcli_function ({ID = "SearchPublicChat",username_ = username},py_username,nil) 
end
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:sadd(TIGER_ID..'LOCK:BAN'..msg.chat_id_,result.sender_user_id_)     
...
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'LOCK:BAN'..msg.chat_id_,result.sender_user_id_)     
...
TIGERBOT:srem(TIGER_ID..'LOCK:BAN'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم الغاء منعه من ( طرد - حظر ) المستخدمين \n',result.id_)   
end    
tdcli_function ({ID = "SearchPublicChat",username_ = username},py_username,nil) 
end
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:sadd(TIGER_ID..'SET:BAN'..msg.chat_id_,result.sender_user_id_)     
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
TIGERBOT:srem(TIGER_ID..'SET:BAN'..msg.chat_id_,result.sender_user_id_)     
...
TIGERBOT:sadd(TIGER_ID..'SET:BAN'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم منحه صلاحية ( طرد - حظر ) المستخدمين \n',result.id_)   
end    
tdcli_function ({ID = "SearchPublicChat",username_ = username},py_username,nil) 
end
...
TIGERBOT:srem(TIGER_ID..'SET:BAN'..msg.chat_id_,result.id_)     
sendMention(msg,msg.chat_id_,'👤┇العضو » {'..CatchNamertprtp(result.type_.user_.first_name_)..'}'..'\n☑┇ تم الغاء منحه صلاحية ( طرد - حظر ) المستخدمين\n',result.id_)   
end    
tdcli_function ({ID = "SearchPublicChat",username_ = username},py_username,nil) 
end
if text ==('مسح صلاحيات الحظر') and is_owner(msg) then 
local list = TIGERBOT:smembers(TIGER_ID..'SET:BAN'..msg.chat_id_) 
if #list == 0 then
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*🤹🏻‍♂️┇* لا يوجد اعضاء لديهم صلاحياة الحظر ليتم مسحهم\n', 1, 'md') 
return false  end
local num = 0
for k,v in pairs(list) do  
TIGERBOT:srem(TIGER_ID..'SET:BAN'..msg.chat_id_,v) 
num = num + 1
end 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1,'*☑┇ تم مسح {'..num..'} من الذين لديهم صلاحيات الحظر *\n', 1, 'md') 
end
...
TIGERBOT:srem(TIGER_ID..'bot:gpsby:id',idch) 
TIGERBOT:srem(TIGER_ID.."bot:gpsby:id:add",idch) 
TIGERBOT:srem(TIGER_ID.."bot:gps:id", idch) 
tdcli_function({ID="GetChat",chat_id_ = '-100'..data.channel_.id_},function(arg,data)  linkgp = TIGERBOT:get(TIGER_ID.."link:group"..idch)  
...
TIGER_sendMsg(SUDO,0, 1,taha, 1, "md")  end,nil)
elseif data.channel_.status_.ID == "ChatMemberStatusMember" then   
print('\27[30;32m»» THE BOT IS NOT ADMIEN ↓\n»» '..'-100'..data.channel_.id_..'\n\27[1;37m')
rem_group('-100'..data.channel_.id_)   
TIGERBOT:sadd(TIGER_ID..'botgps','-100'..data.channel_.id_) 
elseif data.channel_.status_.ID == "ChatMemberStatusEditor" then   
local tahach = '-100'..data.channel_.id_
if TIGERBOT:sismember(TIGER_ID..'bot:gpsby:id:add','-100'..data.channel_.id_)  then  
print('\27[30;33m»» THE GROUP IS HAS BEEN ADD ↓\n»» '..'-100'..data.channel_.id_..'\n\27[1;37m')
else 
print('\27[30;35m»» THE BOT IS ADMIEN AND ADD GROUP ↓\n»» '..'-100'..data.channel_.id_..'\n\27[1;37m')
add_group('-100'..data.channel_.id_)   
TIGERBOT:sadd(TIGER_ID..'bot:gpsby:id','-100'..data.channel_.id_)   
TIGERBOT:sadd(TIGER_ID.."botgps",'-100'..data.channel_.id_)  
TIGERBOT:sadd(TIGER_ID.."bot:gpsby:id:add",'-100'..data.channel_.id_)  
TIGERBOT:set(TIGER_ID.."test:group"..'-100'..data.channel_.id_,'TIGER')    
TIGERBOT:set(TIGER_ID.."add:bot:group"..'-100'..data.channel_.id_, true)   
end
return false 
end  
end
...
TIGERBOT:srem(TIGER_ID.."bot:gpsby:id:add",msg.chat_id_)  
TIGERBOT:srem(TIGER_ID.."botgps",msg.chat_id_)  
TIGERBOT:srem(TIGER_ID..'bot:gpsby:id',msg.chat_id_)   
return false
end
...
delete_msg(msg.chat_id_, {[0] = msg.id_}) 
TIGER_sendMsg(msg.chat_id_, msg.id_, 1, '',1, 'md')  
end
if msg.sender_user_id_ and is_gban(msg.chat_id_,msg.sender_user_id_) then 
kick(msg,msg.chat_id_,msg.sender_user_id_) 
delete_msg(msg.chat_id_, {[0] = msg.id_}) 
local list = TIGERBOT:smembers(TIGER_ID..'bot:gpsby:id')  for k,v in pairs(list) do  kick(msg,v,msg.sender_user_id_) end
return false 
end
...
kick(msg,msg.chat_id_,msg.content_.members_[0].id_) 
delete_msg(msg.chat_id_, {[0] = msg.id_})  
local list = TIGERBOT:smembers(TIGER_ID..'bot:gpsby:id')  for k,v in pairs(list) do  kick(msg,v,msg.content_.members_[0].id_) end
end 
if msg.content_.ID == "MessageChatAddMembers" then  
local mem_id = msg.content_.members_  
for i=0,#mem_id do  
if msg.content_.members_[i].type_.ID == "UserTypeBot" and TIGERBOT:get(TIGER_ID.."lock:Bot:kick"..msg.chat_id_) == 'del'  and not is_vipgroup(msg) then  
kick(msg,msg.chat_id_,mem_id[i].id_)
end  
end  
end
if msg.content_.ID == "MessageChatDeleteMember" and TIGERBOT:get(TIGER_ID..'lock:tagservr'..msg.chat_id_) then
delete_msg(msg.chat_id_,{[0] = msg.id_}) 
end 
if msg.content_.ID == "MessageChatAddMembers" then  
local mem_id = msg.content_.members_  
for i=0,#mem_id do  
if msg.content_.members_[i].type_.ID == "UserTypeBot" and TIGERBOT:get(TIGER_ID.."lock:Bot:kick"..msg.chat_id_) == 'ked' and not is_vipgroup(msg) then  
kick(msg,msg.chat_id_,mem_id[i].id_)
HTTPS.request("https://api.telegram.org/bot" .. chaneel .. "/restrictChatMember?chat_id=" ..msg.chat_id_.. "&user_id=" ..msg.sender_user_id_.."") 
TIGERBOT:sadd(TIGER_ID..'tedmembars'..msg.chat_id_,msg.sender_user_id_)
end  
...
local mem_id = msg.content_.members_  
for i=0,#mem_id do  
if msg.content_.members_[i].type_.ID == "UserTypeBot" and TIGERBOT:get(TIGER_ID.."lock:Bot:kick"..msg.chat_id_) == 'kick' and not is_vipgroup(msg) then  
kick(msg,msg.chat_id_,mem_id[i].id_)
changeChatMemberStatus(msg.chat_id_, msg.sender_user_id_, "Kicked")
end  
end  
end
TIGERadd(data.message_,data)   
TIGER(data.message_,data)   
...
if not is_owner(ta) and TIGERBOT:get(TIGER_ID..'lock:edit'..data.chat_id_) then
...
if not filess then
os.execute('cd requfiles ;wget https://raw.githubusercontent.com/DEVTELE1/TIGER/master/requfiles/JSON.lua') 
dofile('TIGER.lua')  
end
local filess = io.open("requfiles/dkjson.lua","r")
if not filess then
os.execute('cd requfiles ;wget https://raw.githubusercontent.com/DEVTELE1/TIGER/master/requfiles/dkjson.lua') 
dofile('TIGER.lua')  
end
local filess = io.open("requfiles/serpent.lua","r")
if not filess then
os.execute('cd requfiles ;wget https://raw.githubusercontent.com/DEVTELE1/TIGER/master/requfiles/serpent.lua') 
dofile('TIGER.lua')  
end
local list = TIGERBOT:smembers(TIGER_ID.."usersbot")
for k,v in pairs(list) do
...
TIGERBOT:srem(TIGER_ID..'botgps',v) 
...
TIGERBOT:set(TIGER_ID..'group:name'..v,data.title_)
